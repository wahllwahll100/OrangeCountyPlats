...
def render_parcel_card(parcel: dict, idx: int = 0):
    ...
    """
    # ── Field extraction ─────────────────────────────────────────────────
    # The ArcGIS response field names vary by endpoint. We try every known
    # alias in priority order and take the first non-empty match.
    field_map = {
        "parcel_id": ["PARCELNO", "PARCEL_ID", "PARCELID", "PARCEL", "PIN",
                       "PARNO", "PARCEL_NUMBER", "PARID", "PARCEL_NUM"],
        "owner":     ["OWNER", "OWNER1", "OWN_NAME", "OWNERNAME", "OWN_NAME1"],
        "address":   ["SITEADDR", "SITE_ADDR", "ADDRESS", "SITUS",
...
if query_lat and query_lon:
    ...
    if parcels:
        ...
        # Raw data expander with field diagnostic
        with st.expander("📊 Raw Parcel Data & Field Diagnostic"):
            if parcels:
                st.caption("**GIS field names returned** (use this to verify parcel ID mapping):")
                raw_keys = sorted(set(k for p in parcels for k in p.keys()))
                st.code(", ".join(raw_keys), language=None)
                # Show which field resolved as the parcel ID
                test_p = parcels[0]
                pid_candidates = ["PARCELNO", "PARCEL_ID", "PARCELID", "PARCEL", "PIN",
                                  "PARNO", "PARCEL_NUMBER", "PARID", "PARCEL_NUM"]
                matched = [(c, test_p.get(c, "—")) for c in pid_candidates
                           if c in test_p or c.lower() in {k.lower() for k in test_p}]
                if matched:
                    st.caption(f"**Parcel ID resolved from:** `{matched[0][0]}` = `{matched[0][1]}`")
                else:
                    st.warning("⚠ Could not auto-detect the parcel ID field. Check the raw keys above and report to the developer.")
            st.json(parcels)
    else:
