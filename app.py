...
if query_lat and query_lon:
    ...
    if not agent_api_key:
        ...
        # Parcel ID input — user pastes from map results above
        st.caption("Paste the **Parcel ID** from the map results above, then click Research.")
        ...
        with col_btn:
            run_agent = st.button("🔍 Research", use_container_width=True)
        ...
        if run_agent and parcel_id_input:
            ...
            # Build direct links for this parcel
            ...
            # ── Step 1: Fetch parcel data from Florida Statewide Cadastral ──
            # This is an ATTRIBUTE query by parcel ID — no coordinates, no spatial issues.
            parcel_data = {}
            with st.spinner("📡 Fetching parcel data from state records..."):
                try:
                    api_url = (
                        "https://services9.arcgis.com/Gh9awoU677aKree0/arcgis/rest/services/"
                        "Florida_Statewide_Cadastral/FeatureServer/0/query"
                    ...
                    st.caption(f"⚠ State API fetch note: {fetch_err}")

            # Format the raw data for display and agent context
            if parcel_data:
                owner = parcel_data.get("OWN_NAME", "—")
                phy_addr = parcel_data.get("PHY_ADDR1", "—")
                phy_city = parcel_data.get("PHY_CITY", "—")
                phy_zip = str(int(parcel_data.get("PHY_ZIPCD", 0))) if parcel_data.get("PHY_ZIPCD") else "—"
                dor_uc = parcel_data.get("DOR_UC", "—")
                jv = parcel_data.get("JV", 0)
                av_sd = parcel_data.get("AV_SD", 0)
                tv_sd = parcel_data.get("TV_SD", 0)
                lnd_sqfoot = parcel_data.get("LND_SQFOOT", 0)
                act_yr_blt = parcel_data.get("ACT_YR_BLT", "—")
                eff_yr_blt = parcel_data.get("EFF_YR_BLT", "—")
                s_legal = parcel_data.get("S_LEGAL", "—")
                no_buldng = parcel_data.get("NO_BULDNG", "—")
                tot_lvg_ar = parcel_data.get("TOT_LVG_AR", "—")

                data_summary = f"""**State Records Data Retrieved:**
| Field | Value |
|-------|-------|
| **Parcel ID** | {pid_clean} |
| **Owner** | {owner} |
...
| **DOR Use Code** | {dor_uc} |
| **Just Value** | ${jv:,.0f} |
| **Assessed Value** | ${av_sd:,.0f} |
| **Taxable Value** | ${tv_sd:,.0f} |
| **Land Sq Ft** | {lnd_sqfoot:,.0f} |
| **Year Built** | {act_yr_blt} |
| **Living Area** | {tot_lvg_ar} sq ft |
| **Short Legal** | {s_legal} |
"""
                ...
                st.warning("⚠ Could not fetch from state database. The agent will work with the parcel ID only.")
                raw_data_str = "{}"
                data_summary = "No state data retrieved."

            # ── Step 2: OCPA Property Card link (iframe blocked by OCPA) ──
            st.markdown(f"""
            <div class="info-card" style="border-left:4px solid #f7a325;">
                <h3>📄 OCPA Property Card — Find Plat Book & Page Here</h3>
                <div class="role">
                    OCPA blocks iframe embedding, so this opens in a new tab. Look for <strong>"Plat Book"</strong> and <strong>"Plat Page"</strong> 
                    under the Location/Legal section. Write those numbers down — you'll use them in the Comptroller search below to pull the actual plat PDF.
                </div>
                <div style="margin-top:10px;">
                    <a href="{ocpa_card_url}" target="_blank" 
                       style="display:inline-block; background:#0f4c75; color:white; padding:10px 24px; border-radius:6px; font-weight:700; text-decoration:none; font-size:14px;">
                        🔗 Open OCPA Property Card for {pid_clean} →
                    </a>
                </div>
            </div>
            """, unsafe_allow_html=True)

            # ── Step 3: Feed real data to the AI agent for interpretation ──
            with st.spinner("🤖 AI Agent analyzing parcel data..."):
                try:
                    import anthropic
                    client = anthropic.Anthropic(api_key=agent_api_key)

                    system_prompt = f"""You are a plat research analyst for Orange County, Florida. You have REAL parcel data — analyze it and deliver findings. Do NOT give instructions. Do NOT say you can't access websites.

PARCEL ID: {pid_clean}

REAL DATA FROM FLORIDA DOR STATEWIDE DATABASE:
{raw_data_str}

YOUR ANALYSIS TASKS:
1. DECODE THE PARCEL ID: Orange County uses format SS-TT-RR-SSSS-BB-LLL where SS=Section(01-36), TT=Township(20-24), RR=Range(27-34), SSSS=Subdivision code, BB=Block, LLL=Lot. Parse {pid_clean} into these components.

2. INTERPRET THE SHORT LEGAL DESCRIPTION: The S_LEGAL field often contains the subdivision name and lot/block info. Extract the subdivision name from it.

3. PROVIDE A COMPLETE SUMMARY including all data from the table above plus your parcel ID decode.

4. FOR PLAT BOOK/PAGE: The plat book and page are visible on the OCPA Property Card. Tell the user to click the blue "Open OCPA Property Card" button on the page to open it in a new tab, then find "Plat Book" and "Plat Page" in the property details. Once they have those numbers, they click the "Open Comptroller Records Search" button, search by Book/Page, and can view/download the plat PDF for free.

5. HOA/CDD: Based on the subdivision name you extracted from S_LEGAL, note whether this appears to be a platted subdivision that likely has an HOA. Provide the Florida DBPR HOA search link: https://www.myfloridalicense.com/wl11.asp

6. DIRECT LINKS — include these exactly:
   - OCPA Property Card: {ocpa_card_url}
   - OCPA GIS Map: {ocpa_map_url}  
   - Comptroller Records: {comptroller_url}
   - Florida DBPR HOA Search: https://www.myfloridalicense.com/wl11.asp
   - FL Special Districts (CDDs): https://specialdistrictreports.floridajobs.org/webreports/main.aspx

Be direct, factual, and concise. Present data in a clear format. The engineer reading this needs actionable info, not tutorials."""

                    response = client.messages.create(
                        model="claude-haiku-4-5-20251001",
                        max_tokens=1500,
                        messages=[
                            {"role": "user", "content": f"Analyze parcel {pid_clean} using the data provided. Decode the parcel ID, interpret the legal description, and summarize all findings."}
                        ],
                    ...
                    st.error("The `anthropic` package is not installed. Add `anthropic>=0.39.0` to requirements.txt and redeploy.")
                except Exception as e:
                    err_str = str(e)
                    if "credit balance" in err_str or "billing" in err_str.lower():
                        st.error("⚠️ Anthropic billing issue.")
                        st.markdown("👉 [Open Anthropic Billing Page](https://console.anthropic.com/settings/billing)")
                    else:
            ...
            <div class="info-card" style="margin-top:1rem;">
                <h3>🔗 Quick Links for Parcel {pid_clean}</h3>
                <div class="detail">📄 <a href="{ocpa_card_url}" target="_blank">OCPA Property Card</a> — ownership, values, plat book/page, legal description</div>
                <div class="detail">📍 <a href="{ocpa_map_url}" target="_blank">OCPA GIS Map</a> — parcel boundaries, aerial, zoning overlay</div>
                <div class="detail">📑 <a href="{comptroller_url}" target="_blank">OC Comptroller Official Records</a> — search plats, deeds, easements, covenants</div>
                <div class="detail">📱 <a href="https://maps.ocpafl.org/m/Home/ParcelInfo?para=&pid={urllib.parse.quote(pid_clean)}" target="_blank">OCPA Mobile View</a></div>
                <div class="detail">🏘️ <a href="https://www.myfloridalicense.com/wl11.asp" target="_blank">FL DBPR HOA Search</a></div>
                <div class="detail">🏛️ <a href="https://specialdistrictreports.floridajobs.org/webreports/main.aspx" target="_blank">FL Special Districts (CDDs)</a></div>
            </div>
            """, unsafe_allow_html=True)

            # ── Step 5: Direct Plat PDF Link Builder ──
            st.markdown("#### 📑 Plat Document Direct Link Builder")
            st.markdown(f"""
            <div class="info-card" style="border-left:4px solid #f7a325;">
                <h3>⚡ Get the Plat PDF in 2 Steps</h3>
                <div class="role">
                    <strong>Step 1:</strong> Open the OCPA Property Card (button below), find the <strong>Plat Book</strong> and <strong>Plat Page</strong> numbers.<br>
                    <strong>Step 2:</strong> Enter those numbers below → the direct link is generated instantly.
                </div>
                <div style="margin-top:10px;">
                    <a href="{ocpa_card_url}" target="_blank" 
                       style="display:inline-block; background:#0f4c75; color:white; padding:10px 24px; border-radius:6px; font-weight:700; text-decoration:none; font-size:14px;">
                        📄 Open OCPA Property Card → Find Plat Book & Page
                    </a>
                </div>
            </div>
            """, unsafe_allow_html=True)

            st.caption("Enter the Plat Book and Plat Page from the OCPA card:")
            col_book, col_page, col_go = st.columns([2, 2, 2])
            with col_book:
                plat_book = st.text_input("Plat Book:", placeholder="e.g. 1985", key="plat_book_input")
            with col_page:
                plat_page = st.text_input("Plat Page:", placeholder="e.g. 15044", key="plat_page_input")
            with col_go:
                st.markdown("<br>", unsafe_allow_html=True)
                build_link = st.button("🔗 Build Direct Link", use_container_width=True)

            if plat_book and plat_page:
                book_clean = plat_book.strip()
                page_clean = plat_page.strip()

                # Pad page to 6 digits (OC Comptroller format)
                page_padded = page_clean.zfill(6)

                # Construct the direct document URL
                # Format: https://selfservice.or.occompt.com/ssweb/web/integration/document/{BOOK}P{PAGE_PADDED}
                doc_ref = f"{book_clean}P{page_padded}"
                direct_url = f"https://selfservice.or.occompt.com/ssweb/web/integration/document/{doc_ref}"

                # Also build the EagleWeb viewer URL as backup
                eagleweb_url = f"https://officialrecords.occompt.com/recorder/eagleweb/viewDoc.jsp?node=DOC{book_clean}S{page_clean}"

                st.markdown(f"""
                <div class="parcel-result" style="border-left:4px solid #4ade80;">
                    <h3>✅ Direct Plat Document Link</h3>
                    <div class="field"><span class="label">Plat Book:</span><span class="value">{book_clean}</span></div>
                    <div class="field"><span class="label">Plat Page:</span><span class="value">{page_clean}</span></div>
                    <div class="field"><span class="label">Document Ref:</span><span class="value">{doc_ref}</span></div>
                    <div style="margin-top:12px; display:flex; gap:10px; flex-wrap:wrap;">
                        <a href="{direct_url}" target="_blank" 
                           style="display:inline-block; background:#166534; color:white; padding:12px 28px; border-radius:6px; font-weight:700; text-decoration:none; font-size:15px;">
                            📄 Open Plat PDF → {doc_ref}
                        </a>
                        <a href="{eagleweb_url}" target="_blank" 
                           style="display:inline-block; background:#0f4c75; color:white; padding:12px 28px; border-radius:6px; font-weight:700; text-decoration:none; font-size:15px;">
                            📑 Alt: EagleWeb Viewer
                        </a>
                    </div>
                    <div style="margin-top:8px; font-size:11px; color:#64748b;">
                        If the first link doesn't load the correct document, try the EagleWeb Viewer or search manually at
                        <a href="{comptroller_url}" target="_blank">Comptroller Records Search</a>.
                    </div>
                </div>
                """, unsafe_allow_html=True)

            elif build_link:
                st.warning("Enter both Plat Book and Plat Page numbers to generate the link.")

            # Fallback: manual search link
            with st.expander("📑 Manual Comptroller Search (if direct link doesn't work)"):
                st.markdown(f"""
                <div class="resource-card">
                    <div class="res-name">OC Comptroller Self-Service Search</div>
                    <div class="res-desc">Accept disclaimer → Advanced Search → search by Book/Page or Document Type = PLAT</div>
                    <div class="res-url"><a href="https://selfservice.or.occompt.com/ssweb/" target="_blank">https://selfservice.or.occompt.com/ssweb/</a></div>
                </div>
                <div class="resource-card">
                    <div class="res-name">OC Comptroller EagleWeb (Legacy Search)</div>
                    <div class="res-desc">Search by Book/Page, document number, party name, or legal description</div>
                    <div class="res-url"><a href="https://officialrecords.occompt.com/recorder/eagleweb/docSearch.jsp" target="_blank">https://officialrecords.occompt.com/recorder/eagleweb/docSearch.jsp</a></div>
                </div>
                """, unsafe_allow_html=True)

            # ── Step 6: Raw data expander ──
            with st.expander("📊 Raw State Database Fields"):
                if parcel_data:
                    st.json(parcel_data)
                else:
                    st.info("No state data was retrieved for this parcel ID.")

        elif st.session_state.get("agent_parcel_id"):
            # Show previous results context
            prev_pid = st.session_state["agent_parcel_id"]
            st.info(f"Last researched parcel: **{prev_pid}** — enter a new Parcel ID above or click Research again.")
