...
def build_map_html(center_lat=28.5383, center_lon=-81.3792, zoom=11, markers=None, polygon_coords=None, search_address=None):
    ...
    <body>
        ...
        // 1. Census Bureau geocoder → precise lat/lon
        // 2. Spatial parcel query at those coords
        // No server-side geocoding, no bot detection issues.
        async function geocodeAndQuery(rawAddress) {{
            ...
            // Try Census Bureau first (most precise for US addresses)
            try {{
                ...
                if (matches.length > 0) {{
                    ...
                    geocodeSource = 'Census Bureau: ' + matches[0].matchedAddress;
                }}
            }} catch(e) {{
                // Fall through
            }}
            ...
            if (!lat) {{
                try {{
                    ...
                    if (results.length > 0) {{
                        ...
                        geocodeSource = 'Nominatim: ' + results[0].display_name;
                    }}
                }} catch(e) {{
                    // Fall through
                }}
            ...
            document.getElementById('coords').innerHTML =
                '<strong style="color:#f7a325;">GEOCODED:</strong> ' + lat.toFixed(6) + ', ' + lon.toFixed(6) + ' (' + geocodeSource.substring(0,60) + ')';
