...
def build_map_html(center_lat=28.5383, center_lon=-81.3792, zoom=11, markers=None, polygon_coords=None, search_address=None):
    ...
    <body>
        ...
        // Parses the address into components and queries the Florida
        // Statewide Cadastral by PHY_ADDR1 / PHY_CITY / PHY_ZIPCD.
        // This bypasses geocoding entirely — the parcel DB is the
        // source of truth for which parcel has which address.
        // ────────────────────────────────────────────────────────────
        function parseAddress(raw) {{
            // Extract street number, street name, city, zip from common formats
            // "909 Randall St, Orlando, FL 32805" → num=909, street=RANDALL, city=ORLANDO, zip=32805
            ...
            for (var i = 1; i < parts.length; i++) {{
                ...
                // Check if it's a state abbreviation (skip it)
                if (/^[A-Z]{{2}}$/.test(p) || /^FL\s/.test(p) || /^FLORIDA$/.test(p)) {{
                    // May have zip attached: "FL 32805"
                    var zInState = p.match(/(\d{{5}})/);
                    if (zInState) zipPart = zInState[1];
                    continue;
                }}
                // Otherwise treat as city
                if (!cityPart && !/^\d+$/.test(p)) {{
                    cityPart = p.replace(/\s*(FL|FLORIDA)\s*\d*\s*$/i, '').trim();
                }}
            }}

            // Parse street: "909 RANDALL ST" → num=909, name=RANDALL
            var streetMatch = streetPart.match(/^(\d+)\s+(.+)$/);
            var streetNum = '';
            var streetName = '';
            if (streetMatch) {{
                streetNum = streetMatch[1];
                // Remove street type suffixes for flexible matching
                streetName = streetMatch[2]
                    .replace(/\b(ST|STREET|AVE|AVENUE|BLVD|BOULEVARD|DR|DRIVE|RD|ROAD|LN|LANE|CT|COURT|CIR|CIRCLE|WAY|PL|PLACE|TRL|TRAIL|PKWY|PARKWAY|HWY|HIGHWAY)\b\.?/g, '')
                    .trim();
            }}
        ...
        async function queryParcelByAddress(rawAddress) {{
            ...
            // Build WHERE clauses in order of specificity
            var whereClauses = [];

            if (addr.streetNum && addr.streetName) {{
                // Most specific: street number + name fragment + city
                if (addr.city) {{
                    whereClauses.push(
                        "PHY_ADDR1 LIKE '" + addr.streetNum + " " + addr.streetName + "%' AND PHY_CITY='" + addr.city + "'"
                    );
                    // Also try with full street part
                    whereClauses.push(
                        "PHY_ADDR1 LIKE '" + addr.streetFull + "%' AND PHY_CITY='" + addr.city + "'"
                    );
                }}
                if (addr.zip) {{
                    whereClauses.push(
                        "PHY_ADDR1 LIKE '" + addr.streetNum + " " + addr.streetName + "%' AND PHY_ZIPCD='" + addr.zip + "'"
                    );
                }}
                // Broadest: just street
                whereClauses.push(
                    "PHY_ADDR1 LIKE '" + addr.streetNum + " " + addr.streetName + "%'"
                );
            }} else if (addr.streetFull) {{
                if (addr.city) {{
                    whereClauses.push("PHY_ADDR1 LIKE '%" + addr.streetFull + "%' AND PHY_CITY='" + addr.city + "'");
                }}
                whereClauses.push("PHY_ADDR1 LIKE '%" + addr.streetFull + "%'");
            }}
            ...
            if (!features || features.length === 0) {{
                showPanel(
                    '<div class="status-msg warn">⚠ No parcel found matching "' + rawAddress + '" in the statewide database. '
                    ...
                    + '<div class="status-msg info" style="margin-top:6px;font-size:11px;">Queries tried:<br>'
                    + whereClauses.map(function(w){{ return '<code>' + w + '</code>'; }}).join('<br>') + '</div>'
                );
                return;
            }}
