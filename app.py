                ...
                with st.spinner(f"📥 Step 3/4 — Fetching plat PDF ({doc_ref}) from Comptroller..."):
                    ...
                    try:
                        ...
                        if "pdf" in ct or r.content[:4] == b'%PDF':
                            ...
                            for rpat in [r'href=["\']([^"\']*\.pdf[^"\']*)["\']',
                                         r'["\']([^"\']*download[^"\']*document[^"\']*)["\']',
                                         r'["\']([^"\']*getDocument[^"\']*)["\']']:
                                ms = re_mod.findall(rpat, r.text, re_mod.IGNORECASE)
                                if ms:
                                    purl = ms[0] if ms[0].startswith("http") else f"https://selfservice.or.occompt.com{ms[0]}"
                                    pr = requests.get(purl, headers=hdrs, timeout=20, allow_redirects=True)
                    ...
                    # Attempt 2: alt URLs
                    if not pdf_bytes:
                        for alt in [f"{doc_page_url}/pdf", f"{doc_page_url}?format=pdf"]:
                            try:
                                ar = requests.get(alt, headers=hdrs, timeout=15, allow_redirects=True)
