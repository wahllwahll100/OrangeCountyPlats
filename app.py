        ...
        if st.button("🔍 Search Address", use_container_width=True):
            if address_input:
                ...
                if result:
                    ...
                    st.session_state["search_address"] = address_input  # Raw input for DB query
...
with tab_plat:
    ...
    if not agent_api_key:
        st.info("Enter your **Anthropic API key** in the sidebar to enable the AI plat research agent. Without it, you can still use manual OR Book/Page entry below.")

    st.caption("Enter the **Parcel ID** from the map results above, then click **Research & Fetch Plat**.")
    col_pid, col_btn = st.columns([3, 1])
    with col_pid:
        parcel_id_input = st.text_input(
            "Parcel ID:",
            value=st.session_state.get("agent_parcel_id", ""),
            placeholder="e.g., 292235309203120",
            label_visibility="collapsed",
        )
    with col_btn:
        run_agent = st.button("🔍 Research & Fetch Plat", use_container_width=True)

    if run_agent and parcel_id_input:
        st.session_state["agent_parcel_id"] = parcel_id_input
        pid_clean = parcel_id_input.strip().replace("-", "").replace(" ", "")
        ...
        parcel_data = {}
        with st.spinner("📡 Fetching parcel data..."):
            try:
                ...
                }, headers={"User-Agent": "OC-InfraLookup/1.0"}, timeout=20)
                data = resp.json()
        ...
        # Plat series from S_LEGAL (display only)
        plat_book_display = None
        plat_page_display = None
        if s_legal:
            for pat in [r'PB\s*(\d+)\s*[/,\s]*PG\s*(\d+)',
                        r'PLAT\s*BOOK\s*(\d+)\s*[/,\s]*PAGE\s*(\d+)']:
                match = re_mod.search(pat, s_legal.upper())
                if match:
                    plat_book_display = match.group(1)
                    plat_page_display = match.group(2)
                    break

        # OR Book/Page (for Comptroller URL)
        or_book = None
        or_page = None
        ...
        if not or_book:
            with st.spinner("📡 Querying OCPA for OR Book/Page..."):
                ...
                for ep_url, id_fields in ocpa_endpoints:
                    for id_field in id_fields:
                        try:
                            r = requests.get(ep_url, params={
                                ...
                                "outFields": "*", "returnGeometry": "false",
                                "resultRecordCount": "1", "f": "json",
                            }, headers={"User-Agent": "OC-InfraLookup/1.0"}, timeout=12)
                            d = r.json()
        ...
        plat_series_str = f"PB {plat_book_display} / PG {plat_page_display}" if plat_book_display else "—"
        or_str = f"**{or_book}** / **{or_page}**" if or_book and or_page else "⚠ Not auto-detected"
...
| **Legal** | {s_legal} |
| **Plat Series** | {plat_series_str} *(not for Comptroller)* |
| **OR Book / Page** | {or_str} |
""")

        # ── STEP 3: Manual OR entry if not auto-detected ──
        if not or_book or not or_page:
            st.warning("Could not auto-detect **OR Book/Page**. Open the OCPA Property Card to find **Deed Book** and **Deed Page**:")
            ...
            with mc3:
                ...
                manual_go = st.button("📥 Fetch Plat PDF", key="manual_or_fetch")
            if manual_or_book and manual_or_page:
                or_book = manual_or_book.strip()
                or_page = manual_or_page.strip()

        # ── STEP 4: Build Comptroller URL & fetch PDF via Playwright ──
        pdf_bytes = None
        doc_ref = None
        doc_page_url = None
        fetch_errors = []
        ...
        if or_book and or_page:
            ...
            with st.spinner(f"📥 Installing browser engine (first run only)..."):
                pw_ready = ensure_playwright_browsers()

            if pw_ready:
                with st.spinner(f"📥 Fetching plat PDF ({doc_ref}) via headless browser..."):
                    pdf_bytes, fetch_errors = fetch_pdf_with_playwright(doc_page_url)
            else:
                install_err = st.session_state.get("_pw_install_error", "unknown")
                fetch_errors.append(f"Playwright install failed: {install_err}")

                # Fallback: try simple requests
                with st.spinner(f"📥 Trying direct fetch ({doc_ref})..."):
                    hdrs = {
                        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36",
                        "Accept": "application/pdf,*/*",
                    }
                    try:
                        r = requests.get(doc_page_url, headers=hdrs, timeout=20, allow_redirects=True)
                        if r.content[:4] == b'%PDF':
                            pdf_bytes = r.content
                        else:
                            fetch_errors.append(f"Direct fallback: {r.status_code}, {r.headers.get('Content-Type','?')}, {len(r.content)}b")
                    except Exception as e1:
                        fetch_errors.append(f"Direct fallback: {e1}")
        ...
        if pdf_bytes and len(pdf_bytes) > 100:
            ...
            st.info(f"Could not auto-download PDF. Open the plat directly in your browser:")
            ...
            if fetch_errors:
                with st.expander("🔧 Debug: Playwright fetch log"):
                    for err in fetch_errors:
                        st.caption(err)
                    saved_html = st.session_state.get("comptroller_html", "")
                    if saved_html:
                        st.caption("**Page HTML (first 3000 chars):**")
                        st.code(saved_html, language="html")

        # AI Agent (if key provided)
        if agent_api_key and parcel_data:
            with st.spinner("🤖 AI analysis..."):
                try:
                    ...
                    plat_info = f"OR Book {or_book}, Page {or_page} (ref: {doc_ref})" if or_book and or_page else "Not auto-detected"
                    pdf_info = f"PDF retrieved ({len(pdf_bytes):,} bytes)" if pdf_bytes else "PDF not fetched"
                    response = client.messages.create(
                        model="claude-haiku-4-5-20251001",
                        max_tokens=800,
                        messages=[{"role": "user", "content": f"Analyze parcel {pid_clean} for plat lookup purposes."}],
                        system=f"""Plat research analyst for Orange County FL. Focus ONLY on plat-related info. Be concise.
PARCEL: {pid_clean} | OR Book/Page: {plat_info} | Plat Series: {plat_series_str} | PDF: {pdf_info}
S_LEGAL: {s_legal}
Tasks: 1) Decode parcel ID (SS-TT-RR-SSSS-BB-LLL = Section-Township-Range-Subdivision-Block-Lot)
2) Identify subdivision name from legal description
3) Note plat/OR Book status
4) Provide Comptroller URL if OR Book/Page known: https://selfservice.or.occompt.com/ssweb/web/integration/document/{{BOOK}}P{{PAGE_PADDED_6}}
Links: [OCPA Card]({ocpa_card_url}) | [GIS Map]({ocpa_map_url})
Max 100 words. No property value or owner info.""",
                    )
                    ...
                    if "credit" in str(e).lower() or "billing" in str(e).lower():
                        st.error("⚠️ Anthropic billing issue. [Fix billing](https://console.anthropic.com/settings/billing)")
                    else:
        ...
        <div style="margin-top:1rem;padding:12px;background:#f0f4f8;border-radius:8px;font-size:13px;">
            🔗 <a href="{ocpa_card_url}" target="_blank">OCPA Property Card</a> · 
            <a href="{ocpa_map_url}" target="_blank">GIS Map</a> · 
            <a href="https://or.occompt.com/recorder/web/" target="_blank">Comptroller Records</a>
        </div>
        """, unsafe_allow_html=True)
    ...
    elif st.session_state.get("agent_parcel_id"):
        st.info(f"Last researched: **{st.session_state['agent_parcel_id']}** — enter a new Parcel ID or click Research again.")
    else:
        st.markdown("""
        <div class="info-card">
            <h3>How to find a plat</h3>
            <div class="role">
                <strong>1.</strong> Enter an address in the sidebar → the map finds the parcel<br>
                <strong>2.</strong> Copy the <strong>Parcel ID</strong> from the map panel into the field above<br>
                <strong>3.</strong> Click <strong>Research & Fetch Plat</strong> → the tool finds the OR Book/Page and retrieves the plat document from the Comptroller
            </div>
        </div>
        """, unsafe_allow_html=True)
