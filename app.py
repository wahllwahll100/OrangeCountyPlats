...
""")
        ...
        # ── STEP 4: Build Comptroller URL & fetch PDF ──
        pdf_bytes = None
        doc_ref = None
        doc_page_url = None
        fetch_errors = []
        ...
        if or_book and or_page:
            ...
            with st.spinner(f"📥 Fetching plat PDF ({doc_ref})..."):
                hdrs = {
                ...
                try:
                    ...
                    if "pdf" in ct or r.content[:4] == b'%PDF':
                        ...
                        all_urls = re_mod.findall(r'["\']([^"\']{10,})["\']', r.text)
                        for candidate in [u for u in all_urls if any(kw in u.lower() for kw in ['pdf', 'download', 'document', 'content', 'get'])]:
                            ...
                            try:
                                ...
                                pass
                    if not pdf_bytes:
                        fetch_errors.append(f"Direct: {r.status_code}, {ct}, {len(r.content)}b")
                except Exception as e1:
                    fetch_errors.append(f"Fetch: {e1}")

                # Alt URLs
                if not pdf_bytes:
                    for alt in [f"{doc_page_url}/pdf", f"{doc_page_url}/content"]:
                        try:
