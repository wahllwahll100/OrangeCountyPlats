        ...
        if run_agent and parcel_id_input:
            ...
            with st.spinner("🤖 AI Agent researching plat records..."):
                try:
                    ...
                    system_prompt = f"""You are an expert utilities engineer and land records researcher specializing in Orange County, Florida. 
You are helping an engineer quickly find plat documents and subdivision information for a specific parcel.

PARCEL ID: {pid_clean}

DIRECT LINKS (already verified):
- OCPA Property Card: {ocpa_card_url}
- OCPA GIS Map: {ocpa_map_url}
- OC Comptroller Official Records: {comptroller_url}

YOUR TASK:
1. Explain what this parcel ID tells us about the location (the format is Section-Township-Range based).
2. Provide the EXACT steps to retrieve the plat document from the Orange County Comptroller's Official Records:
   - Go to {comptroller_url}
   - Click "Basic Official Records Search"  
   - Search by Document Type = "PLAT" and enter the Plat Book/Page numbers
   - The plat PDF will be viewable and printable for free
3. Explain what the parcel ID format means:
   - Orange County uses a 15-digit format: SS-TT-RR-SSSS-BB-LLL
   - SS = Section (01-36), TT = Township (20-24), RR = Range (27-34)
   - SSSS = Subdivision, BB = Block, LLL = Lot
4. Recommend the user check the OCPA Property Card at the link above to find the specific Plat Book and Page numbers for this parcel, then use those to search the Comptroller's records.
5. Note that the user can also find the recorded subdivision plat by searching the Comptroller's records by the subdivision name.

FORMAT: Use clear headings, numbered steps, and direct links. Be concise and action-oriented — this engineer needs to find the plat fast."""

                    response = client.messages.create(
                        model="claude-sonnet-4-20250514",
                        max_tokens=1500,
                        messages=[
                            {"role": "user", "content": f"Research plat records for Orange County FL parcel {pid_clean}. Give me step-by-step instructions to retrieve the plat document."}
                        ],
