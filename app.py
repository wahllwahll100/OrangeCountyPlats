...
if query_lat and query_lon:
    ...
    if not agent_api_key:
        ...
        if run_agent and parcel_id_input:
            ...
            with st.spinner("🤖 AI Agent researching plat records..."):
                try:
                    import anthropic
                    client = anthropic.Anthropic(api_key=agent_api_key)

                    system_prompt = f"""You are a plat research agent for Orange County, Florida. Your job is to FIND and DELIVER plat information — not to give instructions.

PARCEL ID: {pid_clean}

WHAT YOU MUST DO:
1. Search the web for this parcel's property details on the Orange County Property Appraiser (OCPA) website. 
   Search for: "Orange County FL parcel {pid_clean}" or "ocpafl {pid_clean}" or "site:ocpafl.org {pid_clean}"
2. From the property details, extract:
   - Owner name
   - Site address
   ...
   - Legal description
   - Zoning
3. Once you have the Plat Book and Page, search for the actual plat document:
   Search for: "Orange County FL Comptroller plat book [NUMBER] page [NUMBER]"
   Or: "site:occompt.com plat book [NUMBER]"  
   Or: "[subdivision name] plat Orange County FL"
4. Search for HOA or CDD information:
   Search for: "[subdivision name] HOA Orange County FL"
   Or: "[subdivision name] community development district"

CRITICAL OUTPUT FORMAT — Provide ALL of the following:
- **Parcel ID**: {pid_clean}
- **Owner**: [from search]
- **Address**: [from search]
- **Subdivision**: [from search]  
- **Plat Book / Page**: [from search] 
- **Legal Description**: [from search]
- **Zoning**: [from search]
- **HOA/CDD**: [from search, if found]
- **Direct Links**:
  - OCPA Property Card: {ocpa_card_url}
  - OCPA GIS Map: {ocpa_map_url}
  - Comptroller Records: {comptroller_url}

Do NOT give the user instructions on how to look things up. YOU look it up and return the data. If you cannot find specific information, say what you searched for and what came back."""
                    ...
                    response = client.messages.create(
                        ...
                        messages=[
                            {"role": "user", "content": f"Find the plat records, subdivision name, plat book/page, owner, and HOA info for Orange County FL parcel ID {pid_clean}. Search the web and return the actual data — do not give me instructions."}
                        ],
                        system=system_prompt,
                    )

                    # Extract all text blocks from the response (may include tool_use blocks)
                    agent_text_parts = []
                    ...
                    if agent_response.strip():
                        ...
                        st.warning("Agent returned no text. The search may have failed — try again or check the links below.")
                    ...
                    if "credit balance" in err_str or "billing" in err_str.lower():
                        st.error("⚠️ Anthropic billing issue. Even with credits, you may need a payment method on file.")
                        st.markdown("👉 [Open Anthropic Billing Page](https://console.anthropic.com/settings/billing) — add a payment method, then retry.")
                    elif "tool" in err_str.lower() and "not available" in err_str.lower():
                        # Web search tool not available on this plan — fall back to no-tool version
                        st.warning("Web search tool not available on your API plan. Running without search...")
                        try:
                            response = client.messages.create(
                                model="claude-haiku-4-5-20251001",
                                max_tokens=1500,
                                messages=[
                                    {"role": "user", "content": f"I need plat information for Orange County FL parcel {pid_clean}. The OCPA property card is at {ocpa_card_url}. Decode the parcel ID (format: Section-Township-Range-Subdivision-Block-Lot) and tell me what you can determine about the subdivision and plat from the parcel number structure. Also provide the Comptroller records link: {comptroller_url}"}
                                ],
                            )
                            for block in response.content:
                                if hasattr(block, 'text') and block.text:
                            ...
                            st.error(f"Fallback agent error: {e2}")
                    else:
            ...
            # ── Embedded Comptroller Recorder ──
            st.markdown("#### 📑 Orange County Official Records Search")
            st.caption("Search for plats by Book/Page (from the OCPA Property Card above), by subdivision name, or by document type = PLAT.")
