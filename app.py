...
if query_lat and query_lon:
    ...
    if not agent_api_key:
        ...
        if run_agent and parcel_id_input:
            ...
            # ═══════════════════════════════════════════════════════
            # STEP 2: Extract plat book/page from S_LEGAL
            # ═══════════════════════════════════════════════════════
            import re as re_mod
            ...
            # ── Source 1: Parse S_LEGAL from statewide data ──
            if s_legal:
                for pat in [r'PB\s*(\d+)\s*[/,\s]*PG\s*(\d+)',
                    ...
                    if match:
                        plat_book = match.group(1)
                        plat_page = match.group(2)
                        break

            # ── Source 2: Query OCPA GIS endpoints by parcel ID (attribute query) ──
            # Attribute queries may bypass spatial bot detection
            if not plat_book:
                with st.spinner("📡 Step 2b — Querying OCPA for plat book/page..."):
                    ocpa_plat_endpoints = [
                        # Oakland parcels layer
                        ("https://vgispublic.ocpafl.org/server/rest/services/Oakland/PARCEL/MapServer/0/query",
                         ["PARCELNO", "PARCEL_ID", "PIN", "PID"]),
                        # DynamicForJs land poly
                        ("https://vgispublic.ocpafl.org/server/rest/services/DynamicForJs/OCPA/MapServer/3/query",
                         ["PID"]),
                        # Dynamic OCPA
                        ("https://maps.ocpafl.org/arcgis/rest/services/DynamicOCPA/MapServer/0/query",
                    ...
                    for ep_url, id_fields in ocpa_plat_endpoints:
                        for id_field in id_fields:
                            try:
                                ...
                                if d.get("features") and len(d["features"]) > 0:
                                    ...
                                    # Look for plat book/page in ANY field name
                                    for k, v in attrs.items():
                                        ku = k.upper()
                                        if v and str(v).strip() and str(v) != "0":
                                            if "PLATBOOK" in ku or "PLAT_BOOK" in ku or ku == "PB":
                                                plat_book = str(v).strip()
                                            elif "PLATPAGE" in ku or "PLAT_PAGE" in ku or ku == "PP":
                                                plat_page = str(v).strip()
                                            elif "PLAT_BK" in ku or "PLT_BK" in ku:
                                                plat_book = str(v).strip()
                                            elif "PLAT_PG" in ku or "PLT_PG" in ku:
                                                plat_page = str(v).strip()
                                    # Also check LEGAL/LEGAL_DESC for PB/PG
                                    for k, v in attrs.items():
                                        if plat_book:
                                            break
                                        ku = k.upper()
                                        if ("LEGAL" in ku or "DESCRIPTION" in ku) and v:
                                            legal_str = str(v).upper()
                                            for lpat in [r'PB\s*(\d+)\s*[/,\s]*PG\s*(\d+)',
                                                         r'PLAT\s*BOOK\s*(\d+)\s*[/,\s]*PAGE\s*(\d+)',
                                                         r'BK\s*(\d+)\s*PG\s*(\d+)']:
                                                lm = re_mod.search(lpat, legal_str)
                                                if lm:
                                                    plat_book = lm.group(1)
                                                    plat_page = lm.group(2)
                                                    break
                                    # Store all OCPA data for later use
                                    if attrs:
            ...
            # ── Source 3: Check all raw fields for anything that looks like book/page ──
            if not plat_book and parcel_data:
                for k, v in parcel_data.items():
                    ku = k.upper()
                    vs = str(v).strip() if v else ""
                    if vs and vs != "0" and vs != "None":
                        if "PLAT" in ku and "BOOK" in ku:
                            plat_book = vs
                        elif "PLAT" in ku and "PAGE" in ku:
                            plat_page = vs

            # Display parcel summary
            with st.spinner("📋 Step 2/4 — Analyzing parcel data..."):
                if parcel_data:
                    plat_display = f"**{plat_book}** / **{plat_page}**" if plat_book and plat_page else "⚠ Not found — use manual entry or OCPA card"
                    st.markdown(f"""
| Field | Value |
|-------|-------|
| **Parcel ID** | `{pid_clean}` |
...
| **Just Value** | ${jv:,.0f} |
| **Short Legal** | {s_legal} |
| **Plat Book / Page** | {plat_display} |
""")
                    # Show OCPA fields if we got them
            ...
            # ═══════════════════════════════════════════════════════
            # STEP 3: Auto-construct Comptroller URL and fetch PDF
            # ═══════════════════════════════════════════════════════
            pdf_bytes = None
            doc_ref = None
            doc_page_url = None
            ...
            if plat_book and plat_page:
                page_padded = plat_page.zfill(6)
                doc_ref = f"{plat_book}P{page_padded}"
                ...
                if parcel_data and s_legal:
                    st.warning("⚠ Could not auto-extract Plat Book/Page from the legal description. Use the manual entry below or check the OCPA Property Card.")
            ...
            # Manual fallback if auto-detect missed plat book/page
            if not plat_book or not plat_page:
                with st.expander("✏️ Manual Plat Book/Page Entry"):
                    st.caption("Open the OCPA Property Card, find Plat Book and Page, enter below:")
                    fc1, fc2, fc3 = st.columns([2, 2, 2])
                    with fc1:
                        manual_book = st.text_input("Plat Book:", key="manual_pb")
                    with fc2:
                        manual_page = st.text_input("Plat Page:", key="manual_pp")
                    with fc3:
                        st.markdown("<br>", unsafe_allow_html=True)
                        manual_fetch = st.button("📥 Fetch PDF", key="manual_fetch")
                    if manual_fetch and manual_book and manual_page:
                        mp = manual_page.strip().zfill(6)
                        mref = f"{manual_book.strip()}P{mp}"
                        murl = f"https://selfservice.or.occompt.com/ssweb/web/integration/document/{mref}"
                        try:
                            mr = requests.get(murl, headers={"User-Agent": "Mozilla/5.0", "Accept": "application/pdf,*/*"}, timeout=20, allow_redirects=True)
                            if mr.content[:4] == b'%PDF':
                                st.success(f"✅ PDF: {mref}")
                                st.download_button(f"📥 Download {mref}.pdf", mr.content, f"Plat_{mref}.pdf", "application/pdf", use_container_width=True)
                                import base64
                                b64 = base64.b64encode(mr.content).decode("utf-8")
                                st.markdown(f'<div style="border:2px solid #0f4c75;border-radius:8px;overflow:hidden;margin:10px 0;"><iframe src="data:application/pdf;base64,{b64}" width="100%" height="700px" style="border:none;"></iframe></div>', unsafe_allow_html=True)
                            else:
                                st.markdown(f'[Open {mref} in browser →]({murl})')
                        except Exception as me:
                            st.error(f"Fetch error: {me}")
            ...
            with st.spinner("🤖 Step 4/4 — AI Agent analyzing..."):
                try:
                    ...
                    plat_info = f"Plat Book {plat_book}, Page {plat_page} (ref: {doc_ref})" if plat_book else "Not auto-detected"
...
PARCEL: {pid_clean} | PLAT: {plat_info} | PDF: {pdf_info}
DATA: {raw_data_str}
Tasks: 1) Decode parcel ID (SS-TT-RR-SSSS-BB-LLL) 2) Key facts 3) Plat & PDF status 4) Links: OCPA: {ocpa_card_url} | Map: {ocpa_map_url}
Max 150 words.""",
                    )
