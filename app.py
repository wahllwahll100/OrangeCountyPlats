...
def build_map_html(center_lat=28.5383, center_lon=-81.3792, zoom=11, markers=None, polygon_coords=None, search_address=None):
    ...
    # Auto-query on load: ADDRESS-BASED if we have an address, else spatial
    auto_query_js = ""
    if search_address:
        safe_addr = search_address.replace("\\", "\\\\").replace("'", "\\'")
        auto_query_js = f"setTimeout(function(){{ queryParcelByAddress('{safe_addr}'); }}, 800);"
    elif len(markers) == 1:
