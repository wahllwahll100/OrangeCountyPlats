...
with st.sidebar:
    ...
    # Google Maps API key for precise geocoding
    st.markdown("---")
    st.markdown("#### 🔑 Google Maps API Key")
    st.caption("Optional but **highly recommended** for exact address matching. [Get a free key](https://console.cloud.google.com/apis/credentials) → enable Geocoding API. Free tier: 28,000 requests/month.")
    ...
    # Anthropic API key for AI Plat Research Agent
    st.markdown("#### 🤖 Anthropic API Key")
    st.caption("Powers the AI Plat Research Agent. [Get key](https://console.anthropic.com/settings/keys).")
...
if query_lat and query_lon:
    ...
    # ── AI Plat Research Agent ──────────────────────────────────────────────
    render_section_header("🤖", "AI Plat Research Agent")
    ...
    if not agent_api_key:
        ...
        <div class="info-card">
            ...
            <div class="role">
                Enter your Anthropic API key in the sidebar to activate the AI Plat Research Agent.
                The agent will automatically look up plat documents, subdivision info, and official records
                for any parcel — and provide downloadable links directly in this panel.
                <br><br>
                <a href="https://console.anthropic.com/settings/keys" target="_blank">Get your API key here →</a>
            </div>
        </div>
        """, unsafe_allow_html=True)
    else:
        # Parcel ID input — auto-populated from map or manual
        st.caption("Enter a **Parcel ID** from the map results above, then click **Research & Fetch Plat**.")
        ...
        if run_agent and parcel_id_input:
            ...
            comptroller_url = "https://or.occompt.com/recorder/web/"

            # ═══════════════════════════════════════════════════════
            # STEP 1: Fetch parcel data by PARCEL_ID
            # ═══════════════════════════════════════════════════════
            parcel_data = {}
            with st.spinner("📡 Step 1/4 — Fetching parcel data..."):
                try:
                    ...
                    st.caption(f"⚠ State API note: {fetch_err}")

            # ═══════════════════════════════════════════════════════
            # STEP 2: Extract data fields
            # CRITICAL: Plat Book/Page ≠ OR Book/Page!
            #   - Plat Book/Page (from S_LEGAL, e.g. PB 15 PG 44) = plat series reference
            #   - OR Book/Page (e.g. Book 1985 Page 15044) = Official Records recording ref
            #   The Comptroller URL needs OR Book/Page, NOT Plat Book/Page!
            # ═══════════════════════════════════════════════════════
            import re as re_mod

            s_legal = str(parcel_data.get("S_LEGAL", "")) if parcel_data else ""
            owner = parcel_data.get("OWN_NAME", "—") if parcel_data else "—"
            phy_addr = parcel_data.get("PHY_ADDR1", "—") if parcel_data else "—"
            phy_city = parcel_data.get("PHY_CITY", "—") if parcel_data else "—"
            phy_zip = str(int(parcel_data.get("PHY_ZIPCD", 0))) if parcel_data and parcel_data.get("PHY_ZIPCD") else "—"
            dor_uc = parcel_data.get("DOR_UC", "—") if parcel_data else "—"
            jv = parcel_data.get("JV", 0) if parcel_data else 0

            # Plat Book/Page from S_LEGAL (display only — NOT for Comptroller URL)
            ...
            # OR Book/Page (Official Records) — this is what the Comptroller URL needs
            or_book = None
            or_page = None

            # ── Source 1: Check statewide data for deed/OR fields ──
            if parcel_data:
            ...
            # ── Source 2: Query OCPA GIS endpoints for deed/OR book/page ──
            if not or_book:
                with st.spinner("📡 Step 2b — Querying OCPA for OR Book/Page..."):
                    ocpa_endpoints = [
                        ("https://vgispublic.ocpafl.org/server/rest/services/Oakland/PARCEL/MapServer/0/query",
                                ...
                                if d.get("features") and len(d["features"]) > 0:
                                    ...
                                    for k, v in attrs.items():
                                        ...
                                        if vs and vs != "0" and vs != "None":
                                            # Look for OR/Deed Book (NOT Plat Book!)
                                            if ku in ("DEED_BK", "DEEDBOOK", "DEED_BOOK", "OR_BOOK", "ORBOOK"):
                                                or_book = vs
                                            elif ku in ("DEED_PG", "DEEDPAGE", "DEED_PAGE", "OR_PAGE", "ORPAGE"):
                                                or_page = vs
                                    if or_book:
                                        break
                            except Exception:
            ...
            with st.spinner("📋 Step 2/4 — Analyzing parcel data..."):
                if parcel_data:
                    plat_display = f"PB {plat_book_display} / PG {plat_page_display}" if plat_book_display else "—"
                    or_display = f"**{or_book}** / **{or_page}**" if or_book and or_page else "⚠ Not found — enter manually below"
                    st.markdown(f"""
| Field | Value |
|-------|-------|
| **Parcel ID** | `{pid_clean}` |
| **Owner** | {owner} |
| **Address** | {phy_addr}, {phy_city} {phy_zip} |
| **DOR Use Code** | {dor_uc} |
| **Just Value** | ${jv:,.0f} |
| **Short Legal** | {s_legal} |
| **Plat Book / Page** | {plat_display} *(plat series — NOT for Comptroller)* |
| **OR Book / Page** | {or_display} *(Official Records — for Comptroller URL)* |
""")
                    ocpa_attrs = st.session_state.get("ocpa_attrs")
                    if ocpa_attrs:
                        with st.expander("📋 OCPA GIS fields (from attribute query)"):
                            st.json(ocpa_attrs)
                else:
                    st.warning(f"Could not fetch data for parcel `{pid_clean}` from state database.")

            # ═══════════════════════════════════════════════════════
            # STEP 3: Comptroller URL — uses OR Book/Page ONLY
            # If not auto-detected, user enters OR Book/Page manually
            # (from OCPA Property Card → "Deed Book" and "Deed Page")
            # ═══════════════════════════════════════════════════════
            pdf_bytes = None
            doc_ref = None
            doc_page_url = None
            ...
            # If OR Book/Page not found, show manual entry
            if not or_book or not or_page:
                st.warning("⚠ Could not auto-detect **OR Book/Page** (Official Records reference). The Plat Book/Page from S_LEGAL is a *different* numbering system and cannot be used for the Comptroller URL.")
                st.markdown(f"""
Open the **[OCPA Property Card]({ocpa_card_url})** and find **"Deed Book"** and **"Deed Page"** (or OR Book/Page). Enter them below:
""")
                ...
                if manual_or_book and manual_or_page:
                    ...
                    st.info("Enter both OR Book and OR Page numbers.")
            ...
            if or_book and or_page:
                ...
                with st.spinner(f"📥 Step 3/4 — Fetching plat PDF ({doc_ref}) from Comptroller..."):
                    hdrs = {
                    ...
                    # Attempt 1: Fetch the document page and analyze HTML
                    try:
                        ...
                        if "pdf" in ct or r.content[:4] == b'%PDF':
                            ...
                            html_content = r.text
                            # Scan for ALL URLs in the HTML
                            all_urls = re_mod.findall(r'["\']([^"\']{10,})["\']', html_content)
                            # Filter for PDF-related URLs
                            pdf_candidates = [u for u in all_urls if any(kw in u.lower() for kw in
                                ['pdf', 'download', 'document', 'image', 'content', 'blob', 'file', 'view', 'get'])]
                            for candidate in pdf_candidates:
                                if candidate.startswith("/"):
                                    candidate = f"https://selfservice.or.occompt.com{candidate}"
                                elif not candidate.startswith("http"):
                                    continue
                                try:
                                    pr = requests.get(candidate, headers=hdrs, timeout=15, allow_redirects=True)
                                    if pr.content[:4] == b'%PDF':
                                        pdf_bytes = pr.content
                                        fetch_errors.append(f"Found PDF at: {candidate}")
                                        break
                                except:
                                    pass

                            if not pdf_bytes and r.content[:4] == b'%PDF':
                                pdf_bytes = r.content

                        if not pdf_bytes:
                            fetch_errors.append(f"Direct page: {r.status_code}, {ct}, {len(r.content)}b")
                    except Exception as e1:
                        fetch_errors.append(f"Direct: {e1}")

                    # Attempt 2: Try many common API patterns for document management systems
                    if not pdf_bytes:
                        base = "https://selfservice.or.occompt.com"
                        alt_urls = [
                            f"{base}/ssweb/web/integration/document/{doc_ref}/content",
                            f"{base}/ssweb/web/integration/document/{doc_ref}/download",
                            f"{base}/ssweb/web/integration/document/{doc_ref}/pdf",
                            f"{base}/ssweb/web/integration/document/{doc_ref}/image",
                            f"{base}/ssweb/web/integration/document/{doc_ref}/view",
                            f"{base}/ssweb/api/document/{doc_ref}",
                            f"{base}/ssweb/api/document/{doc_ref}/content",
                            f"{base}/ssweb/api/document/{doc_ref}/download",
                            f"{base}/ssweb/api/document/{doc_ref}/pdf",
                            f"{base}/ssweb/api/document/{doc_ref}/image",
                            f"{base}/ssweb/api/documents/{doc_ref}/content",
                            f"{base}/ssweb/document/{doc_ref}",
                            f"{base}/ssweb/document/{doc_ref}/content",
                            f"{base}/ssweb/documents/{doc_ref}/image",
                            f"{base}/ssweb/web/document/{doc_ref}/content",
                            f"{base}/ssweb/web/document/{doc_ref}/image",
                            f"{doc_page_url}?format=pdf",
                            f"{doc_page_url}&download=true",
                        ]
                        for alt in alt_urls:
                            try:
                                ar = requests.get(alt, headers=hdrs, timeout=10, allow_redirects=True)
                                if ar.content[:4] == b'%PDF':
                                    pdf_bytes = ar.content
                                    fetch_errors.append(f"✅ Found PDF at: {alt}")
                                    break
                                elif ar.status_code < 400:
                                    fetch_errors.append(f"{alt}: {ar.status_code}, {ar.headers.get('Content-Type','?')[:30]}, {len(ar.content)}b")
                                else:
                                    fetch_errors.append(f"{alt}: {ar.status_code}")
                            except Exception as ae:
                                fetch_errors.append(f"{alt}: {ae}")

                    if pdf_bytes:
                        st.session_state["last_plat_pdf"] = pdf_bytes
                        st.session_state["last_plat_ref"] = doc_ref
                    else:
                        # Store HTML for debugging
                        st.session_state["comptroller_html"] = html_content[:5000] if html_content else ""
            ...
            if pdf_bytes and len(pdf_bytes) > 100:
                ...
                st.warning(f"⚠ Could not auto-download PDF for {doc_ref}. Open it directly in your browser:")
                st.markdown(f"""
                <a href="{doc_page_url}" target="_blank"
                ...
                if fetch_errors:
                    with st.expander("🔧 Debug: fetch attempts"):
                        ...
                        if saved_html:
                            st.caption("**Comptroller page HTML (first 5000 chars):**")
                            st.code(saved_html, language="html")

            # AI Agent analysis (concise, no instructions)
            with st.spinner("🤖 Step 4/4 — AI Agent analyzing..."):
                try:
                    import anthropic
                    client = anthropic.Anthropic(api_key=agent_api_key)
                    raw_data_str = json.dumps(parcel_data, indent=2, default=str) if parcel_data else "{}"
                    plat_info = f"OR Book {or_book}, Page {or_page} (ref: {doc_ref})" if or_book and or_page else "OR Book/Page not auto-detected"
                    plat_series = f"PB {plat_book_display} / PG {plat_page_display}" if plat_book_display else "N/A"
                    pdf_info = f"PDF retrieved ({len(pdf_bytes):,} bytes)" if pdf_bytes else "PDF not fetched"
                    response = client.messages.create(
                        model="claude-haiku-4-5-20251001",
                        max_tokens=1000,
                        messages=[{"role": "user", "content": f"Analyze parcel {pid_clean}. Decode ID, summarize data, note plat status."}],
                        system=f"""Plat research analyst for Orange County FL. Be concise. No instructions.
PARCEL: {pid_clean} | OR Book/Page: {plat_info} | Plat Series: {plat_series} | PDF: {pdf_info}
NOTE: Plat Book/Page (e.g. PB 15 PG 44) is the plat series ref. OR Book/Page (e.g. 1985/15044) is the Official Records recording ref used in the Comptroller URL.
Comptroller URL format: https://selfservice.or.occompt.com/ssweb/web/integration/document/{{BOOK}}P{{PAGE_PADDED_6}}
DATA: {raw_data_str}
Tasks: 1) Decode parcel ID (SS-TT-RR-SSSS-BB-LLL) 2) Key facts 3) OR Book/Page & PDF status 4) Links: OCPA: {ocpa_card_url} | Map: {ocpa_map_url}
Max 150 words.""",
                    )
                    ...
                    err_s = str(e)
                    if "credit balance" in err_s or "billing" in err_s.lower():
                        st.error("⚠️ Anthropic billing issue.")
                        st.markdown("👉 [Billing Page](https://console.anthropic.com/settings/billing)")
                    else:
            ...
            <div class="info-card" style="margin-top:1rem;">
                <h3>🔗 Quick Links for {pid_clean}</h3>
                <div class="detail">📄 <a href="{ocpa_card_url}" target="_blank">OCPA Property Card</a></div>
                <div class="detail">📍 <a href="{ocpa_map_url}" target="_blank">OCPA GIS Map</a></div>
                <div class="detail">📑 <a href="{comptroller_url}" target="_blank">Comptroller Records</a></div>
                <div class="detail">🏘️ <a href="https://www.myfloridalicense.com/wl11.asp" target="_blank">FL HOA Search</a></div>
            </div>
            """, unsafe_allow_html=True)
            with st.expander("📊 Raw State Database Fields"):
                if parcel_data:
                    st.json(parcel_data)

        elif st.session_state.get("agent_parcel_id"):
            prev_pid = st.session_state["agent_parcel_id"]
            st.info(f"Last researched: **{prev_pid}** — enter a new Parcel ID or click Research again.")
