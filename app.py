...
<style>
    ...
    section[data-testid="stSidebar"] [data-testid="stAlert"][data-baseweb-alert-kind="success"] {
        background: #14532d !important;
    }
    section[data-testid="stSidebar"] [data-testid="stAlert"][data-baseweb-alert-kind="warning"] {
        background: #713f12 !important;
    }
    section[data-testid="stSidebar"] [data-testid="stAlert"][data-baseweb-alert-kind="error"] {
...
# ── Playwright PDF Fetcher ───────────────────────────────────────────────────

def ensure_playwright_browsers():
    """Install Playwright Chromium if not already present. Cached via session state."""
    if st.session_state.get("_pw_installed"):
        return True
    try:
        import subprocess
        ...
        st.session_state["_pw_installed"] = True
        return True
    except Exception as e:
        st.session_state["_pw_install_error"] = str(e)
        return False


def fetch_pdf_with_playwright(url, timeout_ms=30000):
    """
    Use a headless browser to navigate to the Comptroller document page,
    wait for the PDF to load, and capture the PDF bytes.
    
    Strategy:
    1. Intercept all network responses — capture any PDF that the page fetches
    2. If no PDF intercepted, look for iframe/embed src pointing to a PDF
    3. As last resort, look for a download link and follow it
    
    Returns: (pdf_bytes, debug_info) tuple
    """
    ...
    try:
        with sync_playwright() as p:
            ...
            # Intercept all network responses to capture PDF content
            def handle_response(response):
                try:
                    ct = response.headers.get("content-type", "").lower()
                    url_str = response.url
                    if "pdf" in ct or url_str.lower().endswith(".pdf"):
                        body = response.body()
                        if body and (body[:4] == b'%PDF' or "pdf" in ct):
                            captured_pdfs.append(body)
                            debug_info.append(f"✅ Captured PDF from network: {url_str[:100]} ({len(body):,}b)")
                except Exception:
                    pass
            ...
            # Give JS a moment to finish loading the PDF viewer
            page.wait_for_timeout(3000)
            
            # Check if we captured a PDF from network traffic
            if captured_pdfs:
                # Take the largest one (the actual document, not thumbnails)
                pdf_bytes = max(captured_pdfs, key=len)
                debug_info.append(f"Got PDF from network interception ({len(pdf_bytes):,} bytes)")
            
            # Strategy 2: Look for iframe or embed with PDF src
            if not pdf_bytes:
                for selector in ["iframe[src*='pdf']", "iframe[src*='document']", 
                    ...
                    try:
                        ...
                        if el:
                            ...
                            if src and src.startswith("http"):
                                debug_info.append(f"Found embedded PDF src: {src[:100]}")
                                resp = page.request.get(src)
                                if resp.body()[:4] == b'%PDF':
                                    pdf_bytes = resp.body()
                                    debug_info.append(f"Got PDF from embedded element ({len(pdf_bytes):,} bytes)")
                                    break
                    except Exception:
                        pass
            
            # Strategy 3: Look for download/view links on the page
            if not pdf_bytes:
                for link_sel in ["a[href*='pdf']", "a[href*='download']", "a[href*='document']",
                                 "a[href*='content']", "a[href*='image']", "button"]:
                    try:
                        links = page.query_selector_all(link_sel)
                        for link in links[:5]:
                            href = link.get_attribute("href") or ""
                            text = link.inner_text().lower()
                            if href and ("pdf" in href.lower() or "download" in text or "view" in text):
                                if href.startswith("/"):
                                    href = f"https://selfservice.or.occompt.com{href}"
                                debug_info.append(f"Trying link: {href[:100]}")
                                try:
                                    resp = page.request.get(href)
                                    if resp.body()[:4] == b'%PDF':
                                        pdf_bytes = resp.body()
                                        debug_info.append(f"Got PDF from link ({len(pdf_bytes):,} bytes)")
                                        break
                                except Exception:
                                    pass
                        if pdf_bytes:
            ...
            # Strategy 4: Try clicking any visible "download" or "view" buttons
            if not pdf_bytes:
                for btn_text in ["Download", "View", "PDF", "Print", "Open"]:
                    try:
                        btn = page.get_by_text(btn_text, exact=False).first
                        if btn and btn.is_visible():
                            debug_info.append(f"Clicking button: '{btn_text}'")
                            ...
                            if pdf_bytes[:4] == b'%PDF':
                                debug_info.append(f"Got PDF from download ({len(pdf_bytes):,} bytes)")
                            else:
                                pdf_bytes = None
                            break
                    except Exception:
            ...
            # Capture page HTML for debugging if no PDF found
            if not pdf_bytes:
                html_content = page.content()
                debug_info.append(f"Page title: {page.title()}")
                debug_info.append(f"Page HTML size: {len(html_content)} chars")
                # Store first 3000 chars for debug
                st.session_state["comptroller_html"] = html_content[:3000]
            
            browser.close()
            
    except Exception as e:
        debug_info.append(f"Playwright error: {e}")
    
    return pdf_bytes, debug_info


def fetch_or_book_page_from_ocpa(parcel_id):
    """
    Use Playwright to load the OCPA property card page and extract
    the OR Book/Page (Deed Book/Page) from the rendered HTML.
    Returns: (or_book, or_page, debug_info)
    """
    from playwright.sync_api import sync_playwright
    import re as _re
    ...
    url = f"https://ocpaweb.ocpafl.org/parcelsearch/Parcel%20ID/{parcel_id}"
    debug_info.append(f"Loading OCPA card: {url}")
    ...
    try:
        with sync_playwright() as p:
            ...
            html = page.content()
            debug_info.append(f"Page loaded: {len(html)} chars")
            
            # Strategy 1: Look for labeled fields
            # Common patterns on OCPA pages:
            #   "Deed Book" ... "1985"   "Deed Page" ... "15044"
            #   "Plat Book" ... "67"     "Plat Page" ... "10"  (wrong — this is plat series)
            #   "OR Book" ... "1985"     "OR Page" ... "15044"
            
            # We want DEED Book/Page or OR Book/Page, NOT Plat Book/Page
            patterns_book = [
                r'Deed\s*Book[^0-9]*?(\d{3,})',
                r'OR\s*Book[^0-9]*?(\d{3,})',
                r'Official\s*Records?\s*Book[^0-9]*?(\d{3,})',
                r'Recording\s*Book[^0-9]*?(\d{3,})',
            ]
            patterns_page = [
                r'Deed\s*Page[^0-9]*?(\d{3,})',
                r'OR\s*Page[^0-9]*?(\d{3,})',
                r'Official\s*Records?\s*Page[^0-9]*?(\d{3,})',
                r'Recording\s*Page[^0-9]*?(\d{3,})',
            ]
            
            for pat in patterns_book:
                m = _re.search(pat, html, _re.IGNORECASE)
                if m:
                    or_book = m.group(1)
                    debug_info.append(f"Found OR Book: {or_book} (pattern: {pat[:30]})")
                    break
            
            for pat in patterns_page:
                m = _re.search(pat, html, _re.IGNORECASE)
                if m:
                    or_page = m.group(1)
                    debug_info.append(f"Found OR Page: {or_page} (pattern: {pat[:30]})")
                    break
            
            # Strategy 2: Try using Playwright to read specific elements
            if not or_book:
                for selector in ["td", "span", "div", "dd"]:
                    elements = page.query_selector_all(selector)
                    for i, el in enumerate(elements):
                        try:
                            text = el.inner_text().strip()
                            if _re.search(r'Deed\s*Book', text, _re.IGNORECASE):
                                # The value might be in the NEXT sibling or same element
                                nums = _re.findall(r'(\d{3,})', text)
                                if nums:
                                    or_book = nums[0]
                                    debug_info.append(f"Found Book in element: {text[:60]}")
                                else:
                                    # Check next sibling
                                    next_el = elements[i+1] if i+1 < len(elements) else None
                                    if next_el:
                                        next_text = next_el.inner_text().strip()
                                        nums = _re.findall(r'(\d{3,})', next_text)
                                        if nums:
                                            or_book = nums[0]
                                            debug_info.append(f"Found Book in next element: {next_text[:60]}")
                            elif _re.search(r'Deed\s*Page', text, _re.IGNORECASE):
                                nums = _re.findall(r'(\d{3,})', text)
                                if nums:
                                    or_page = nums[0]
                                    debug_info.append(f"Found Page in element: {text[:60]}")
                                else:
                                    next_el = elements[i+1] if i+1 < len(elements) else None
                                    if next_el:
                                        next_text = next_el.inner_text().strip()
                                        ...
                                        if nums:
                                            or_page = nums[0]
                                            debug_info.append(f"Found Page in next element: {next_text[:60]}")
                        except Exception:
                            pass
            ...
            if not or_book:
                ...
                debug_info.append(f"Page text sample: {page_text[:500]}")
                st.session_state["ocpa_page_text"] = page_text
            
            browser.close()
    except Exception as e:
        debug_info.append(f"OCPA scrape error: {e}")
    
    return or_book, or_page, debug_info


# ── Map Generation with Folium ──────────────────────────────────────────────

def build_map_html(center_lat=28.5383, center_lon=-81.3792, zoom=11, markers=None, polygon_coords=None, search_address=None, google_api_key=None):
    """Generate a Leaflet map with CLIENT-SIDE parcel querying.
    
    Two query modes:
      1. ADDRESS-BASED (primary when user searches an address):
         Queries parcel DB with a WHERE clause on the address field.
         No geocoding imprecision — the DB returns the exact parcel.
      2. SPATIAL (when user clicks the map):
         Point-in-polygon intersection — always correct since the user
         clicked the actual parcel they want.
    """
    markers = markers or []
    marker_js = ""
    for m in markers:
    ...
    # Auto-query: pass the raw address to the browser. The JavaScript will:
    # 1. Geocode via Census Bureau API (from the user's browser — no server blocks)
    # 2. Spatial query at the precise coordinates
    # This is the approach that was working — spatial query at good coords.
    auto_query_js = ""
    raw_addr = st.session_state.get("search_address", "")
    if raw_addr:
        safe_addr = raw_addr.replace("\\", "\\\\").replace("'", "\\'").replace('"', '\\"')
        auto_query_js = f"setTimeout(function(){{ geocodeAndQuery('{safe_addr}'); }}, 600);"
    elif len(markers) == 1:
        auto_query_js = f"setTimeout(function(){{ queryParcelSpatial({markers[0]['lat']}, {markers[0]['lon']}); }}, 800);"
    ...
    <body>
        ...
        // Two modes:
        //   1. queryParcelByAddress(addr) – queries parcel DB by address text (WHERE clause)
        //      This is EXACT. No geocoding. The DB finds the parcel by its own address field.
        //   2. queryParcelSpatial(lat, lon) – point-in-polygon (for map clicks)

        // Endpoints that support attribute (WHERE) queries on address fields
        var ADDR_ENDPOINT = 'https://services9.arcgis.com/Gh9awoU677aKree0/arcgis/rest/services/Florida_Statewide_Cadastral/FeatureServer/0/query';

        // Endpoints for spatial queries (map clicks)
        var SPATIAL_ENDPOINTS = [
            'https://vgispublic.ocpafl.org/server/rest/services/DynamicForJs/OCPA/MapServer/3/query',
            'https://vgispublic.ocpafl.org/server/rest/services/Oakland/PARCEL/MapServer/0/query',
            'https://maps.ocpafl.org/arcgis/rest/services/DynamicOCPA/MapServer/0/query',
            ADDR_ENDPOINT
        ...
        function showLoading(msg) {{
            ...
            document.getElementById('results-panel').innerHTML = '<div class="status-msg info">⏳ ' + (msg || 'Querying parcel data...') + '</div>';
        }}

        // ────────────────────────────────────────────────────────────
        // MODE 1: ADDRESS-BASED QUERY (primary for sidebar searches)
        //
        // Field types in Florida Statewide Cadastral (critical!):
        //   PHY_ADDR1 = String (e.g. "909 RANDALL ST")
        //   PHY_CITY  = String (e.g. "ORLANDO")
        //   PHY_ZIPCD = DOUBLE (e.g. 32805) — NO quotes in WHERE!
        //   CO_NO     = DOUBLE (Orange County = 48) — NO quotes!
        //   PARCEL_ID = String
        //
        // Strategy: try progressively broader WHERE clauses, then
        // fall back to spatial query at geocoded coordinates.
        // ────────────────────────────────────────────────────────────
        function parseAddress(raw) {{
            ...
            for (var i = 1; i < parts.length; i++) {{
                ...
                // Skip state-only tokens
                if (/^(FL|FLORIDA)(\\\\s+\\\\d{{5}})?$/.test(p)) {{
                    var zInState = p.match(/(\\\\d{{5}})/);
                    if (zInState) zipPart = zInState[1];
                    continue;
                }}
                // City (first non-zip, non-state token)
                if (!cityPart && p.length > 1 && !/^\\\\d+$/.test(p)) {{
                    cityPart = p.replace(/\\\\s*(FL|FLORIDA)\\\\s*\\\\d*\\\\s*$/i, '').trim();
                }}
            }}

            // Parse street — keep the FULL street including type (ST, AVE, etc.)
            // "909 RANDALL ST" → num=909, streetFull="909 RANDALL ST"
            var streetMatch = streetPart.match(/^(\\\\d+)\\\\s+(.+)$/);
            var streetNum = streetMatch ? streetMatch[1] : '';
            var streetRest = streetMatch ? streetMatch[2].trim() : streetPart;

            // Also create a version without street type for fallback
            var streetNameOnly = streetRest
                .replace(/\\\\b(ST|STREET|AVE|AVENUE|BLVD|BOULEVARD|DR|DRIVE|RD|ROAD|LN|LANE|CT|COURT|CIR|CIRCLE|WAY|PL|PLACE|TRL|TRAIL|PKWY|PARKWAY|HWY|HIGHWAY)\\\\b\\\\.?\\\\s*$/g, '')
                .trim();

            return {{
                streetNum: streetNum,
                streetRest: streetRest,        // "RANDALL ST"
                streetNameOnly: streetNameOnly, // "RANDALL"
                streetFull: streetPart,          // "909 RANDALL ST"
                city: cityPart,
                zip: zipPart
            }};
        }}

        async function queryParcelByAddress(rawAddress) {{
            showLoading('Searching parcel database for: ' + rawAddress);

            var addr = parseAddress(rawAddress);
            var whereClauses = [];

            // Orange County CO_NO = 48 (Double field — no quotes)
            var countyFilter = ' AND CO_NO=48';

            // ── Build WHERE clauses from most specific to broadest ──

            // 1. Exact match: full address + city + county
            if (addr.streetFull && addr.city) {{
                whereClauses.push("PHY_ADDR1='" + addr.streetFull + "' AND PHY_CITY='" + addr.city + "'" + countyFilter);
            }}

            // 2. LIKE with full street + city + county
            if (addr.streetFull && addr.city) {{
                whereClauses.push("PHY_ADDR1 LIKE '" + addr.streetFull + "%' AND PHY_CITY='" + addr.city + "'" + countyFilter);
            }}

            // 3. Full street + zip (PHY_ZIPCD is Double — no quotes!)
            if (addr.streetFull && addr.zip) {{
                whereClauses.push("PHY_ADDR1 LIKE '" + addr.streetFull + "%' AND PHY_ZIPCD=" + addr.zip + countyFilter);
            }}

            // 4. Number + street name (without type) + city
            if (addr.streetNum && addr.streetNameOnly && addr.city) {{
                whereClauses.push("PHY_ADDR1 LIKE '" + addr.streetNum + " " + addr.streetNameOnly + "%' AND PHY_CITY='" + addr.city + "'" + countyFilter);
            }}

            // 5. Number + street name (without type) + county only
            if (addr.streetNum && addr.streetNameOnly) {{
                whereClauses.push("PHY_ADDR1 LIKE '" + addr.streetNum + " " + addr.streetNameOnly + "%'" + countyFilter);
            }}

            // 6. Full street + county only (no city filter)
            if (addr.streetFull) {{
                whereClauses.push("PHY_ADDR1 LIKE '" + addr.streetFull + "%'" + countyFilter);
            }}
            ...
            for (var w = 0; w < whereClauses.length; w++) {{
                try {{
                    var url = ADDR_ENDPOINT
                        + '?where=' + encodeURIComponent(whereClauses[w])
                        + '&outFields=*'
                        + '&returnGeometry=true'
                        + '&outSR=4326'
                        + '&resultRecordCount=5'
                    ...
                    if (data.error) {{
                        errors.push(whereClauses[w] + ' → ERROR: ' + (data.error.message || JSON.stringify(data.error)));
                        continue;
                    }}

                    if (data.features && data.features.length > 0) {{
                        features = data.features;
                        successWhere = whereClauses[w];
                        break;
                    }} else {{
                        errors.push(whereClauses[w] + ' → 0 results');
                    }}
                }} catch(e) {{
                    errors.push(whereClauses[w] + ' → ' + e.message);
                    continue;
                }}
            }}

            // ── FALLBACK: if no address match, try spatial query at geocoded coords ──
            if ((!features || features.length === 0) && map.getCenter()) {{
                var c = map.getCenter();
                errors.push('Falling back to spatial query at ' + c.lat.toFixed(6) + ', ' + c.lng.toFixed(6));
                try {{
                    var spatialUrl = ADDR_ENDPOINT
                        ...
                        + '&returnGeometry=true'
                        + '&f=json';
                    var spatResp = await fetch(spatialUrl, {{ signal: AbortSignal.timeout(15000) }});
                    var spatData = await spatResp.json();
                    if (spatData.features && spatData.features.length > 0) {{
                        features = spatData.features;
                        successWhere = 'Spatial fallback at geocoded coords';
                    }}
                }} catch(e) {{
                    errors.push('Spatial fallback failed: ' + e.message);
                }}
            }}

            // ── FALLBACK 2: try OCPA endpoints spatially ──
            if ((!features || features.length === 0) && map.getCenter()) {{
                var c2 = map.getCenter();
                for (var s = 0; s < SPATIAL_ENDPOINTS.length; s++) {{
                    try {{
                        var ocpaUrl = SPATIAL_ENDPOINTS[s]
                            + '?geometry=' + c2.lng + ',' + c2.lat
                            + '&geometryType=esriGeometryPoint'
                            + '&spatialRel=esriSpatialRelIntersects'
                            + '&inSR=4326&outSR=4326&where=1%3D1&outFields=*&returnGeometry=false&f=json';
                        var ocpaResp = await fetch(ocpaUrl, {{ signal: AbortSignal.timeout(12000) }});
                        var ocpaData = await ocpaResp.json();
                        if (ocpaData.features && ocpaData.features.length > 0) {{
                            features = ocpaData.features;
                            successWhere = 'OCPA spatial at ' + extractHost(SPATIAL_ENDPOINTS[s]);
                            break;
                        }}
                    }} catch(e) {{ continue; }}
                }}
            ...
            if (!features || features.length === 0) {{
                showPanel(
                    '<div class="status-msg warn">⚠ No parcel found matching "' + rawAddress + '". '
                    + 'Try clicking the correct parcel on the map directly, or use: '
                    + '<a href="https://ocpaweb.ocpafl.org/parcelsearch" target="_blank">OCPA Search</a></div>'
                    + '<details style="margin-top:6px;font-size:11px;color:#64748b;"><summary>Debug: queries attempted</summary>'
                    + '<pre style="font-size:10px;max-height:200px;overflow:auto;background:#f1f5f9;padding:8px;border-radius:4px;">'
                    + errors.join('\\n') + '</pre></details>'
                );
                return;
            }}

            // If geometry was returned, zoom the map to the parcel centroid
            if (features[0].geometry) {{
                ...
                if (geom.rings) {{
                    ...
                    var sumX = 0, sumY = 0;
                    for (var r = 0; r < ring.length; r++) {{ sumX += ring[r][0]; sumY += ring[r][1]; }}
                    centLon = sumX / ring.length;
                    centLat = sumY / ring.length;
                }} else if (geom.x) {{
                    centLon = geom.x; centLat = geom.y;
                }}
                if (centLat && centLon) {{
                    map.setView([centLat, centLon], 17);
                    if (queryMarker) map.removeLayer(queryMarker);
                    queryMarker = L.marker([centLat, centLon], {{
                        icon: L.divIcon({{
            ...
            renderParcelResults(features, 'Address query: ' + successWhere);
        }}

        // ────────────────────────────────────────────────────────────
        // MODE 2: SPATIAL QUERY (for map clicks)
        // User clicked a specific point – find whichever parcel
        // polygon contains that point. Always correct.
        // ────────────────────────────────────────────────────────────
        async function queryParcelSpatial(lat, lon) {{
            showLoading('Querying parcel at ' + lat.toFixed(6) + ', ' + lon.toFixed(6));
            ...
            for (var i = 0; i < SPATIAL_ENDPOINTS.length; i++) {{
                try {{
                    var url = SPATIAL_ENDPOINTS[i]
                        + '?geometry=' + lon + ',' + lat
                        + '&geometryType=esriGeometryPoint'
                        + '&spatialRel=esriSpatialRelIntersects'
                        + '&inSR=4326&outSR=4326'
                    ...
                    if (data.features && data.features.length > 0) {{
                        features = data.features;
                        successEndpoint = SPATIAL_ENDPOINTS[i];
                        break;
                    }}
                }} catch(e) {{
                    continue;
            ...
            if (!features || features.length === 0) {{
                showPanel(
                    '<div class="status-msg warn">⚠ No parcel found at this location. '
                    + 'This may be outside Orange County or on unplatted land.</div>'
                    + '<div class="status-msg info" style="margin-top:8px;">'
                    + '🔗 Try: <a href="https://vgispublic.ocpafl.org/webmapjs/" target="_blank">OCPA GIS Map</a>'
                    + ' · <a href="https://ocpaweb.ocpafl.org/parcelsearch" target="_blank">OCPA Search</a></div>'
                );
                return;
            }}

            renderParcelResults(features, extractHost(successEndpoint));
        }}

        // ────────────────────────────────────────────────────────────
        // SHARED: Render parcel results into the panel
        // ────────────────────────────────────────────────────────────
        function renderParcelResults(features, source) {{
            var html = '<div class="status-msg ok">✓ ' + features.length + ' parcel(s) found · <em style="font-size:11px;">' + source + '</em></div>';
            ...
            for (var f = 0; f < features.length; f++) {{
                ...
                var deedBk = findAttr(a, ['DEED_BK','DEEDBOOK','OR_BOOK','BOOK','BK']) || '';
                var deedPg = findAttr(a, ['DEED_PG','DEEDPAGE','OR_PAGE','PAGE','PG']) || '';
                ...
                if (deedBk && deedPg) {{
                    html += row('OR Book/Page', deedBk + ' / ' + deedPg);
                    var pgPad = String(deedPg).padStart(6, '0');
                    var docRef = deedBk + 'P' + pgPad;
                    var directDocUrl = 'https://selfservice.or.occompt.com/ssweb/web/integration/document/' + docRef;
                    html += '<div class="link-row" style="margin:8px 0;padding:8px;background:#dcfce7;border-radius:6px;">';
                    html += '<a href="' + directDocUrl + '" target="_blank" style="color:#166534;font-weight:700;font-size:13px;">📄 Open Plat PDF → ' + docRef + '</a></div>';
                }}

                if (pid !== 'N/A') {{
                    html += '<div class="links-section">';
                    html += '<div class="link-title">🔗 Direct Links:</div>';
                ...
                // Auto-fetch OR Book/Page from OCPA mobile (runs in browser)
                html += '<div id="plat-lookup-' + f + '" style="margin-top:6px;"></div>';
                html += '</div>';

                if (pid !== 'N/A' && !deedBk) {{
                    (function(pidVal, idx) {{
                        setTimeout(function() {{
                            var el = document.getElementById('plat-lookup-' + idx);
                            if (!el) return;
                            el.innerHTML = '<span style="color:#64748b;font-size:11px;">🔍 Fetching plat reference from OCPA...</span>';

                            fetch('https://maps.ocpafl.org/m/Home/ParcelInfo?para=&pid=' + pidVal)
                                .then(function(r) {{ return r.text(); }})
                                .then(function(pageHtml) {{
                                    // Extract ALL numbers near book/page/deed keywords
                                    var bk = null, pg = null;

                                    // Try multiple patterns on the HTML
                                    var patterns = [
                                        /Deed\\s*Book[\\s:]*(\\d+)/i,
                                        /OR\\s*Book[\\s:]*(\\d+)/i,
                                        /Plat\\s*Book[\\s:]*(\\d+)/i,
                                    ...
                                    for (var p = 0; p < patterns.length; p++) {{
                                        var m = pageHtml.match(patterns[p]);
                                        if (m && m[1] && parseInt(m[1]) > 100) {{ bk = m[1]; break; }}
                                    }}
                                    for (var p = 0; p < pgPatterns.length; p++) {{
                                        var m = pageHtml.match(pgPatterns[p]);
                                        if (m && m[1] && parseInt(m[1]) > 100) {{ pg = m[1]; break; }}
                                    }}

                                    // Also try: "Book/Page: 1985 / 15044" pattern
                                    if (!bk) {{
                                        var bpMatch = pageHtml.match(/Book\\s*\\/\\s*Page[\\s:]*(\\d{{3,}})\\s*\\/\\s*(\\d{{3,}})/i);
                                        if (bpMatch) {{ bk = bpMatch[1]; pg = bpMatch[2]; }}
                                    }}

                                    // Try to find in table cells: <td>Deed Book</td><td>1985</td>
                                    if (!bk) {{
                                        var tdMatch = pageHtml.match(/Deed\\s*Book<\\/td>\\s*<td[^>]*>\\s*(\\d+)/i) ||
                                                      pageHtml.match(/Plat\\s*Book<\\/td>\\s*<td[^>]*>\\s*(\\d+)/i);
                                        if (tdMatch) bk = tdMatch[1];
                                    }}
                                    if (!pg) {{
                                        var tdPgMatch = pageHtml.match(/Deed\\s*Page<\\/td>\\s*<td[^>]*>\\s*(\\d+)/i) ||
                                                        pageHtml.match(/Plat\\s*Page<\\/td>\\s*<td[^>]*>\\s*(\\d+)/i);
                                        if (tdPgMatch) pg = tdPgMatch[1];
                                    }}

                                    // Try label/value pairs: "Deed Book:" or "Deed Book</b>" followed by number
                                    if (!bk) {{
                                        var lblMatch = pageHtml.match(/Deed\\s*Book[^<]*?(\\d{{3,}})/i) ||
                                                       pageHtml.match(/Plat\\s*Book[^<]*?(\\d{{3,}})/i);
                                        if (lblMatch) bk = lblMatch[1];
                                    }}
                                    if (!pg) {{
                                        var lblPgMatch = pageHtml.match(/Deed\\s*Page[^<]*?(\\d{{3,}})/i) ||
                                                         pageHtml.match(/Plat\\s*Page[^<]*?(\\d{{3,}})/i);
                                        if (lblPgMatch) pg = lblPgMatch[1];
                                    }}

                                    if (bk && pg) {{
                                        var pgPad = String(pg).padStart(6, '0');
                                        var ref = bk + 'P' + pgPad;
                                        var url = 'https://selfservice.or.occompt.com/ssweb/web/integration/document/' + ref;
                                        el.innerHTML = '<div style="padding:10px;background:#dcfce7;border-radius:6px;border:1px solid #86efac;">' +
                                            '<strong style="color:#166534;font-size:13px;">✅ Plat Document: OR Book ' + bk + ' / Page ' + pg + '</strong><br>' +
                                            '<a href="' + url + '" target="_blank" style="display:inline-block;margin-top:6px;background:#166534;color:white;padding:8px 20px;border-radius:6px;font-weight:700;text-decoration:none;font-size:14px;">' +
                                            '📄 Open Plat PDF → ' + ref + '</a></div>';
                                    }} else {{
                                        // Show what we found for debugging
                                        var snippet = pageHtml.substring(0, 3000);
                                        var bookNums = snippet.match(/\\b(\\d{{3,5}})\\b/g) || [];
                                        el.innerHTML = '<div style="padding:8px;background:#fef3c7;border-radius:6px;border:1px solid #fde68a;font-size:12px;">' +
                                            '⚠ Could not auto-detect OR Book/Page. <a href="https://ocpaweb.ocpafl.org/parcelsearch/Parcel%20ID/' + pidVal + '" target="_blank" style="font-weight:700;">Open OCPA Property Card</a> to find Deed Book/Page, then enter below.' +
                                            '</div>';
                                    }}
                                }})
                                .catch(function(err) {{
                                    el.innerHTML = '<span style="color:#92400e;font-size:11px;">⚠ OCPA mobile unavailable. <a href="https://ocpaweb.ocpafl.org/parcelsearch/Parcel%20ID/' + pidVal + '" target="_blank">Open OCPA Card</a></span>';
                                }});
                        }}, 600);
                    }})(pidClean, f);
                }}
        ...
        // ── CLIENT-SIDE GEOCODING + SPATIAL QUERY ──
        // Priority: Google Maps → Census Bureau → Nominatim
        // All run in the user's browser — no server blocks.
        var GOOGLE_API_KEY = '{google_api_key or ""}';

        async function geocodeAndQuery(rawAddress) {{
            showLoading('Geocoding: ' + rawAddress);

            var lat = null, lon = null;
            var geocodeSource = '';

            // ── Try 1: Google Maps Geocoding API (most accurate) ──
            if (GOOGLE_API_KEY) {{
                try {{
                    var gUrl = 'https://maps.googleapis.com/maps/api/geocode/json'
                        + '?address=' + encodeURIComponent(rawAddress)
                        + '&key=' + GOOGLE_API_KEY;
                    var gResp = await fetch(gUrl, {{ signal: AbortSignal.timeout(8000) }});
                    var gData = await gResp.json();
                    if (gData.status === 'OK' && gData.results && gData.results.length > 0) {{
                        var loc = gData.results[0].geometry.location;
                        lat = loc.lat;
                        lon = loc.lng;
                        geocodeSource = 'Google: ' + gData.results[0].formatted_address;
                    }}
                }} catch(e) {{
                    // Fall through
                }}
            ...
            // ── Try 2: Census Bureau (precise for US addresses) ──
            if (!lat) {{
                try {{
                    var censusUrl = 'https://geocoding.geo.census.gov/geocoder/locations/onelineaddress'
                        + '?address=' + encodeURIComponent(rawAddress)
                        + '&benchmark=Public_AR_Current&format=json';
                    var resp = await fetch(censusUrl, {{ signal: AbortSignal.timeout(10000) }});
                    var data = await resp.json();
                    var matches = (data.result && data.result.addressMatches) ? data.result.addressMatches : [];
                    if (matches.length > 0) {{
                        lat = matches[0].coordinates.y;
                        lon = matches[0].coordinates.x;
                        geocodeSource = 'Census: ' + matches[0].matchedAddress;
                    }}
                }} catch(e) {{
                    // Fall through
                }}
            ...
            // ── Try 3: Nominatim (broadest fallback) ──
            if (!lat) {{
                try {{
                    var nomUrl = 'https://nominatim.openstreetmap.org/search'
                        + '?q=' + encodeURIComponent(rawAddress)
                        + '&format=json&addressdetails=1&limit=1&countrycodes=us';
                    var resp2 = await fetch(nomUrl, {{
                        headers: {{ 'User-Agent': 'OC-InfraLookup/1.0' }},
                        signal: AbortSignal.timeout(8000)
                    }});
                    var results = await resp2.json();
                    if (results.length > 0) {{
                        lat = parseFloat(results[0].lat);
                        lon = parseFloat(results[0].lon);
                        geocodeSource = 'Nominatim (less precise): ' + results[0].display_name;
                    }}
                }} catch(e) {{
                    // Fall through
                }}
            ...
            if (!lat || !lon) {{
                showPanel('<div class="status-msg warn">⚠ Could not geocode "' + rawAddress + '". Try clicking the parcel on the map directly.</div>');
                return;
            }}
            ...
            // Show geocode source in the coordinate bar
            document.getElementById('coords').innerHTML =
                '<strong style="color:#f7a325;">GEOCODED:</strong> ' + lat.toFixed(6) + ', ' + lon.toFixed(6)
                + '<br><span style="font-size:10px;color:#94a3b8;">' + geocodeSource.substring(0, 80) + '</span>';

            // Now do spatial parcel query at the precise coordinates
            await queryParcelSpatial(lat, lon);
        }}

        // ── Click to query (spatial – user clicked the parcel they want) ──
        map.on('click', function(e) {{
            document.getElementById('coords').innerHTML =
                '<strong style="color:#f7a325;">QUERYING:</strong> ' + e.latlng.lat.toFixed(6) + ', ' + e.latlng.lng.toFixed(6);
            queryParcelSpatial(e.latlng.lat, e.latlng.lng);
        }});
...
<div class="app-header">
    ...
    <h1>🏗️ PlatQuest 9000</h1>
    <p>Address → Parcel ID → Development Plat Document</p>
</div>
""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    # API Keys — loaded from Streamlit Secrets if available, otherwise manual input
    st.markdown("---")
    # Try secrets first (set in .streamlit/secrets.toml or Streamlit Cloud settings)
    try:
        secrets_google = st.secrets.get("GOOGLE_MAPS_API_KEY", "")
    except Exception:
        secrets_google = ""
    try:
    ...
    if secrets_google:
        st.session_state["google_api_key"] = secrets_google
        st.markdown('<span style="color:#4ade80;font-size:12px;">✓ Google key loaded from secrets</span>', unsafe_allow_html=True)
    else:
        st.markdown("#### 🔑 Google Maps API Key")
        st.caption("[Get a free key](https://console.cloud.google.com/apis/credentials) or add `GOOGLE_MAPS_API_KEY` to `.streamlit/secrets.toml`")
        ...
        if google_api_key:
            st.markdown('<span style="color:#4ade80;font-size:12px;">✓ Google Geocoding active</span>', unsafe_allow_html=True)

    if secrets_anthropic:
        st.session_state["anthropic_api_key"] = secrets_anthropic
        st.markdown('<span style="color:#4ade80;font-size:12px;">✓ Anthropic key loaded from secrets</span>', unsafe_allow_html=True)
    else:
        st.markdown("#### 🤖 Anthropic API Key")
        st.caption("[Get key](https://console.anthropic.com/settings/keys) or add `ANTHROPIC_API_KEY` to `.streamlit/secrets.toml`")
        ...
        if anthropic_api_key:
            st.markdown('<span style="color:#4ade80;font-size:12px;">✓ AI Agent active</span>', unsafe_allow_html=True)
    st.markdown("---")
    ...
    if input_method == "Address Search":
        ...
        address_input = st.text_input(
            "Enter address:",
            placeholder="e.g., 763 Golden Sunshine Cir, Orlando FL",
        )
        if st.button("🔍 Search", use_container_width=True):
            if address_input:
                ...
                if result:
                    ...
                    st.caption(f"📡 {source} · `{query_lat:.6f}, {query_lon:.6f}`")

                    # ── Auto-find Parcel ID by address ──
                    with st.spinner("🔍 Finding parcel..."):
                        parsed_addr = address_input.strip().upper()
                        # Extract street part (before first comma)
                        street_part = parsed_addr.split(",")[0].strip()
                        
                        api_url = ("https://services9.arcgis.com/Gh9awoU677aKree0/arcgis/rest/services/"
                                   "Florida_Statewide_Cadastral/FeatureServer/0/query")
                        
                        # Try progressively broader queries
                        where_clauses = [
                            f"PHY_ADDR1='{street_part}' AND CO_NO=48",
                            f"PHY_ADDR1 LIKE '{street_part}%' AND CO_NO=48",
                        ]
                        # Also try without street type
                        import re as _re
                        street_no_type = _re.sub(
                            r'\b(ST|STREET|AVE|AVENUE|BLVD|DR|DRIVE|RD|ROAD|LN|LANE|CT|COURT|CIR|CIRCLE|WAY|PL|PLACE|TRL|TRAIL|PKWY|HWY)\b\.?\s*$',
                            '', street_part).strip()
                        if street_no_type != street_part:
                            where_clauses.append(f"PHY_ADDR1 LIKE '{street_no_type}%' AND CO_NO=48")
                        
                        # Spatial fallback
                        where_clauses.append("__SPATIAL__")
                        ...
                        for wc in where_clauses:
                            try:
                                if wc == "__SPATIAL__":
                                    params = {
                                        "geometry": f"{query_lon},{query_lat}",
                                        "geometryType": "esriGeometryPoint",
                                        "spatialRel": "esriSpatialRelIntersects",
                                        "inSR": "4326", "outSR": "4326",
                                        "where": "1=1",
                                ...
                                resp = requests.get(api_url, params=params, 
                                    headers={"User-Agent": "PlatQuest/1.0"}, timeout=15)
                                data = resp.json()
                                if data.get("features") and len(data["features"]) > 0:
                                    found_pid = data["features"][0]["attributes"].get("PARCEL_ID", "")
                                    if found_pid:
                                        break
                            except Exception:
                                continue
                        ...
                        if found_pid:
                            st.session_state["agent_parcel_id"] = found_pid
                            st.session_state["auto_run_plat"] = True
                            st.success(f"✓ Parcel: `{found_pid}`")
                        else:
                            st.warning("Could not auto-find parcel. Click the correct parcel on the map.")
                else:
        ...
        if st.button("🔍 Search Coordinates", use_container_width=True):
            if coord_input:
                ...
                if parsed:
                    ...
                    st.session_state["search_address"] = None  # No address query for coords
...
map_html = build_map_html(
    ...
    search_address=st.session_state.get("search_address"),
...
# ── Two-Tab Layout ──────────────────────────────────────────────────────────
tab_plat, tab_contacts = st.tabs(["📄 Plat Search", "📞 Contacts & Resources"])


# ═══════════════════════════════════════════════════════════════════════════
# TAB 1: PLAT SEARCH — fully automated
# ═══════════════════════════════════════════════════════════════════════════
with tab_plat:

    agent_api_key = st.session_state.get("anthropic_api_key", "")
    pid_from_session = st.session_state.get("agent_parcel_id", "")
    auto_run = st.session_state.pop("auto_run_plat", False)

    # Show current parcel if we have one
    if pid_from_session and auto_run:
        st.markdown(f"**Auto-detected Parcel ID:** `{pid_from_session}`")

    # Manual override / fallback
    with st.expander("✏️ Manual Parcel ID entry (if auto-detect missed)", expanded=not pid_from_session):
        col_pid, col_btn = st.columns([3, 1])
        with col_pid:
            manual_pid = st.text_input(
                "Parcel ID:",
                value=pid_from_session,
                placeholder="e.g., 292235309203120",
                label_visibility="collapsed",
            )
        with col_btn:
            manual_run = st.button("🔍 Fetch Plat", use_container_width=True)
        if manual_run and manual_pid:
            pid_from_session = manual_pid.strip()
            st.session_state["agent_parcel_id"] = pid_from_session
            auto_run = True

    # ── RUN THE PIPELINE ──
    if pid_from_session and auto_run:
        pid_clean = pid_from_session.strip().replace("-", "").replace(" ", "")
        ocpa_card_url = f"https://ocpaweb.ocpafl.org/parcelsearch/Parcel%20ID/{urllib.parse.quote(pid_clean)}"
        ocpa_map_url = f"https://vgispublic.ocpafl.org/webmapjs/?pid={urllib.parse.quote(pid_clean)}"

        # ── STEP 1: Fetch parcel data ──
        parcel_data = {}
        with st.spinner("📡 Step 1/4 — Fetching parcel data..."):
            try:
                api_url = ("https://services9.arcgis.com/Gh9awoU677aKree0/arcgis/rest/services/"
                           "Florida_Statewide_Cadastral/FeatureServer/0/query")
                resp = requests.get(api_url, params={
                    "where": f"PARCEL_ID='{pid_clean}'",
                    "outFields": "*", "returnGeometry": "false",
                    "resultRecordCount": "1", "f": "json",
                }, headers={"User-Agent": "PlatQuest/1.0"}, timeout=20)
                data = resp.json()
                if data.get("features") and len(data["features"]) > 0:
                    parcel_data = data["features"][0].get("attributes", {})
            except Exception as fetch_err:
                st.caption(f"⚠ State API: {fetch_err}")

        import re as re_mod
        s_legal = str(parcel_data.get("S_LEGAL", "")) if parcel_data else ""
        phy_addr = parcel_data.get("PHY_ADDR1", "—") if parcel_data else "—"
        phy_city = parcel_data.get("PHY_CITY", "—") if parcel_data else "—"

        # ── STEP 2: Find OR Book/Page ──
        or_book = None
        or_page = None
        or_debug = []

        # Source 1: Check statewide data for deed fields
        if parcel_data:
            for k, v in parcel_data.items():
                ku = k.upper()
                vs = str(v).strip() if v else ""
                if vs and vs != "0" and vs != "None":
                    if ("DEED" in ku and "BOOK" in ku) or ku in ("OR_BOOK", "ORBOOK"):
                        or_book = vs
                    elif ("DEED" in ku and "PAGE" in ku) or ku in ("OR_PAGE", "ORPAGE"):
                        or_page = vs

        # Source 2: OCPA GIS attribute query
        if not or_book:
            with st.spinner("📡 Step 2/4 — Querying OCPA for OR Book/Page..."):
                for ep_url, id_fields in [
                    ("https://vgispublic.ocpafl.org/server/rest/services/Oakland/PARCEL/MapServer/0/query",
                     ["PARCELNO", "PARCEL_ID", "PIN", "PID"]),
                    ("https://maps.ocpafl.org/arcgis/rest/services/DynamicOCPA/MapServer/0/query",
                     ["PARCELNO", "PARCEL_ID", "PIN"]),
                ]:
                    for id_field in id_fields:
                        try:
                            r = requests.get(ep_url, params={
                                "where": f"{id_field}='{pid_clean}'",
                                "outFields": "*", "returnGeometry": "false",
                                "resultRecordCount": "1", "f": "json",
                            }, headers={"User-Agent": "PlatQuest/1.0"}, timeout=12)
                            d = r.json()
                            if d.get("features") and len(d["features"]) > 0:
                                attrs = d["features"][0].get("attributes", {})
                                st.session_state["ocpa_attrs"] = attrs
                                for k, v in attrs.items():
                                    ku = k.upper()
                                    vs = str(v).strip() if v else ""
                                    if vs and vs != "0":
                                        if ku in ("DEED_BK", "DEEDBOOK", "DEED_BOOK", "OR_BOOK"):
                                            or_book = vs
                                        elif ku in ("DEED_PG", "DEEDPAGE", "DEED_PAGE", "OR_PAGE"):
                                            or_page = vs
                                if or_book:
                                    or_debug.append(f"Got OR Book/Page from OCPA GIS: {or_book}/{or_page}")
                                    break
                        except Exception:
                            continue
                    if or_book:
        ...
        # Source 3: Playwright scrapes OCPA property card
        if not or_book:
            with st.spinner("🌐 Step 2b/4 — Loading OCPA property card (headless browser)..."):
                pw_ready = ensure_playwright_browsers()
                if pw_ready:
                    or_book, or_page, ocpa_debug = fetch_or_book_page_from_ocpa(pid_clean)
                    or_debug.extend(ocpa_debug)
                else:
                    or_debug.append("Playwright not available for OCPA scrape")

        # Show compact summary
        plat_series_str = "—"
        if s_legal:
            pb_match = re_mod.search(r'PB\s*(\d+)\s*[/,\s]*PG\s*(\d+)', s_legal.upper())
            if pb_match:
                plat_series_str = f"PB {pb_match.group(1)} / PG {pb_match.group(2)}"

        or_str = f"**{or_book}** / **{or_page}**" if or_book and or_page else "⚠ Not found"
        st.markdown(f"""
| | |
|---|---|
| **Parcel ID** | `{pid_clean}` |
| **Address** | {phy_addr}, {phy_city} |
| **Legal** | {s_legal} |
| **Plat Series** | {plat_series_str} |
| **OR Book / Page** | {or_str} |
""")

        # ── STEP 3: Manual OR fallback ──
        if not or_book or not or_page:
            st.warning("Could not auto-detect OR Book/Page. Enter manually from the OCPA Property Card:")
            st.markdown(f"""<a href="{ocpa_card_url}" target="_blank" 
                style="display:inline-block;background:#0f4c75;color:white;padding:8px 20px;border-radius:6px;font-weight:700;text-decoration:none;font-size:13px;">
                📄 Open OCPA Property Card →</a>""", unsafe_allow_html=True)
            mc1, mc2, mc3 = st.columns([2, 2, 2])
            with mc1:
                manual_or_book = st.text_input("OR / Deed Book:", placeholder="e.g. 1985", key="manual_or_book")
            with mc2:
                manual_or_page = st.text_input("OR / Deed Page:", placeholder="e.g. 15044", key="manual_or_page")
            with mc3:
                st.markdown("<br>", unsafe_allow_html=True)
                if st.button("📥 Fetch PDF", key="manual_or_fetch"):
                    pass  # triggers rerun with values
            if manual_or_book and manual_or_page:
                or_book = manual_or_book.strip()
                or_page = manual_or_page.strip()

        # ── STEP 4: Fetch plat PDF via Playwright ──
        pdf_bytes = None
        doc_ref = None
        doc_page_url = None
        fetch_errors = list(or_debug)

        if or_book and or_page:
            page_padded = or_page.zfill(6)
            doc_ref = f"{or_book}P{page_padded}"
            doc_page_url = f"https://selfservice.or.occompt.com/ssweb/web/integration/document/{doc_ref}"

            with st.spinner(f"📥 Step 3/4 — Fetching plat PDF ({doc_ref}) via headless browser..."):
                pw_ready = ensure_playwright_browsers()
                if pw_ready:
                    pdf_bytes, pdf_debug = fetch_pdf_with_playwright(doc_page_url)
                    fetch_errors.extend(pdf_debug)
                else:
                    fetch_errors.append("Playwright not available")
                    # Fallback to requests
                    try:
                        r = requests.get(doc_page_url, headers={
                            "User-Agent": "Mozilla/5.0", "Accept": "application/pdf,*/*"
                        }, timeout=20, allow_redirects=True)
                        if r.content[:4] == b'%PDF':
                            pdf_bytes = r.content
                        else:
                            fetch_errors.append(f"Direct: {r.status_code}, {len(r.content)}b")
                    except Exception as e1:
                        fetch_errors.append(f"Direct: {e1}")

        # ── Display PDF ──
        if pdf_bytes and len(pdf_bytes) > 100:
            st.success(f"✅ Plat PDF retrieved: **{doc_ref}** ({len(pdf_bytes):,} bytes)")
            st.download_button(
            ...
            st.markdown(f"""
            <div style="border:2px solid #0f4c75; border-radius:8px; overflow:hidden; margin:10px 0;">
                <iframe src="data:application/pdf;base64,{b64_pdf}"
                        width="100%" height="700px" style="border:none;"></iframe>
            </div>
            """, unsafe_allow_html=True)
        elif doc_ref:
            st.info(f"Could not auto-download PDF. Open directly:")
            st.markdown(f"""<a href="{doc_page_url}" target="_blank"
                style="display:inline-block;background:#166534;color:white;padding:12px 28px;border-radius:6px;font-weight:700;text-decoration:none;font-size:15px;margin:8px 0;">
                📄 Open Plat → {doc_ref}</a>""", unsafe_allow_html=True)

        # Debug — always visible
        if fetch_errors:
            with st.expander("🔧 Debug: fetch log", expanded=False):
                for err in fetch_errors:
                    st.caption(err)
                saved_html = st.session_state.get("comptroller_html", "")
                if saved_html:
                    st.caption("**Comptroller page HTML:**")
                    st.code(saved_html, language="html")
                ocpa_text = st.session_state.get("ocpa_page_text", "")
                if ocpa_text:
                    st.caption("**OCPA page text:**")
                    st.code(ocpa_text[:2000])

        # AI Agent
        if agent_api_key and parcel_data:
            with st.spinner("🤖 Step 4/4 — AI analysis..."):
                try:
                    import anthropic
                    client = anthropic.Anthropic(api_key=agent_api_key)
                    raw_data_str = json.dumps(parcel_data, indent=2, default=str)
                    plat_info = f"OR Book {or_book}, Page {or_page} (ref: {doc_ref})" if or_book and or_page else "Not detected"
                    pdf_info = f"PDF retrieved ({len(pdf_bytes):,} bytes)" if pdf_bytes else "PDF not fetched"
                    response = client.messages.create(
                        model="claude-haiku-4-5-20251001",
                        max_tokens=800,
                        messages=[{"role": "user", "content": f"Analyze parcel {pid_clean} for plat lookup."}],
                        system=f"""Plat research analyst for Orange County FL. Focus ONLY on plat info. Be concise.
PARCEL: {pid_clean} | OR: {plat_info} | Plat Series: {plat_series_str} | PDF: {pdf_info}
S_LEGAL: {s_legal}
Decode parcel ID, identify subdivision, note plat status, provide Comptroller URL if known.
Links: [OCPA Card]({ocpa_card_url}) | [GIS Map]({ocpa_map_url})
Max 100 words.""",
                    )
                    ...
                    if "credit" in str(e).lower() or "billing" in str(e).lower():
                        st.error("⚠️ Anthropic billing issue. [Fix](https://console.anthropic.com/settings/billing)")
                    else:
                        st.error(f"Agent error: {e}")

        # Quick links
        st.markdown(f"""
        <div style="margin-top:1rem;padding:12px;background:#f0f4f8;border-radius:8px;font-size:13px;">
            🔗 <a href="{ocpa_card_url}" target="_blank">OCPA Card</a> · 
            <a href="{ocpa_map_url}" target="_blank">GIS Map</a> · 
            <a href="https://or.occompt.com/recorder/web/" target="_blank">Comptroller</a>
        </div>
        """, unsafe_allow_html=True)
    ...
    elif pid_from_session and not auto_run:
        st.info(f"Parcel **{pid_from_session}** ready. Click **Fetch Plat** above or search a new address.")
    else:
        st.markdown("""
        <div class="info-card">
            <h3>How it works</h3>
            <div class="role">
                Enter an address in the sidebar and click <strong>Search</strong>.<br>
                The tool automatically finds the parcel, looks up the OR Book/Page,
                and fetches the plat PDF from the Comptroller — all in one click.
            </div>
        </div>
        """, unsafe_allow_html=True)

# ═══════════════════════════════════════════════════════════════════════════
# TAB 2: CONTACTS & RESOURCES
# ═══════════════════════════════════════════════════════════════════════════
with tab_contacts:

    st.markdown("### 🔗 Engineering Resource URLs")
    st.caption("Direct links to tools, portals, and databases for OC project research.")
    ...
    resource_keys = [
        "gis_and_mapping", "permits_and_development", "plats_and_surveys",
        "utilities_and_infrastructure", "environmental_and_stormwater",
        "hoa_and_cdd", "transportation",
    ]
...
<div class="disclaimer">
    <strong>⚠️ Disclaimer:</strong> Reference information only — verify with authoritative sources before use in
    engineering design or permit applications. Not an official government application.
</div>
<div style="text-align:center; padding:1rem 0 0.5rem; color:#94a3b8; font-size:0.75rem;">
    OC PlatQuest 9000 v2.0 · Orange County, Florida
</div>
""", unsafe_allow_html=True)
