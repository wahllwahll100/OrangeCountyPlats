...
def query_oc_parcels(lat: float, lon: float, radius_m: float = 50) -> list:
    ...
    parcels = _query_arcgis(_STATEWIDE_PARCELS_URL, {
        ...
        "inSR": "4326",
        "outSR": "4326",
        "where": "CO_NO=95",          # Orange County FIPS
        "outFields": "*",
...
if query_lat and query_lon:
    ...
    if parcels:
        ...
        st.markdown('<span class="status-warn">⚠ No parcel data returned from any GIS endpoint.</span>', unsafe_allow_html=True)
        with st.expander("🔧 Debug: Endpoints Attempted"):
            st.caption("**Primary endpoint** (Florida Statewide Cadastral on ArcGIS Online):")
            test_url = f"{_STATEWIDE_PARCELS_URL}?geometry={query_lon},{query_lat}&geometryType=esriGeometryPoint&spatialRel=esriSpatialRelIntersects&inSR=4326&outSR=4326&where=CO_NO=95&outFields=PARCEL_ID,OWN_NAME,PHY_ADDR1&returnGeometry=false&f=json"
            st.code(test_url, language=None)
            st.caption("Copy the URL above and paste it in your browser to test. If it returns features, the issue is Streamlit Cloud networking. If it returns an error, the service may be down.")
            try:
                headers = {"User-Agent": "OC-InfraLookup/1.0 (Streamlit)"}
                test_resp = requests.get(_STATEWIDE_PARCELS_URL, params={
                ...
                st.caption(f"**Response status:** {test_resp.status_code}")
