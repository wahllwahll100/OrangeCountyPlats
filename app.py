...
def query_oc_parcels(lat: float, lon: float, radius_m: float = 50) -> list:
    ...
    Tries multiple verified OCPA endpoints in order of reliability.
    """
    parcels = []
    
    # Endpoint 1: OCPA public GIS server (verified working)
    endpoints = [
        ...
        "https://gis.orangecountyfl.net/arcgis/rest/services/Dynamic/Parcels/MapServer/0/query",
    ]
    ...
    for endpoint_url in endpoints:
        try:
            ...
            resp = requests.get(endpoint_url, params=params, timeout=15)
            data = resp.json()
...
def query_parcels_by_envelope(min_lon, min_lat, max_lon, max_lat) -> list:
    """Query parcels within a bounding box using multiple verified endpoints."""
    parcels = []
    endpoints = [
        "https://vgispublic.ocpafl.org/server/rest/services/Oakland/PARCEL/MapServer/0/query",
        "https://maps.ocpafl.org/arcgis/rest/services/DynamicOCPA/MapServer/0/query",
...
def query_parcels_by_polygon(rings: list) -> list:
    """Query parcels intersecting a polygon using multiple verified endpoints."""
    parcels = []
...
def render_parcel_card(parcel: dict, idx: int = 0):
    """Render a single parcel result card with direct deep-links to OCPA, Map, and Comptroller."""
    # Common field name mappings (ArcGIS field names vary across OCPA endpoints)
    field_map = {
        "parcel_id": ["PARCELNO", "PARCEL_ID", "PARCELID", "PARCEL", "PIN", "PARNO", "PARCEL_NUMBER"],
        "owner": ["OWNER", "OWNER1", "OWN_NAME", "OWNERNAME", "OWN_NAME1"],
        "address": ["SITEADDR", "SITE_ADDR", "ADDRESS", "SITUS", "SITUS_ADDR", "SITUSADDRESS", "SITUS_ADDRESS"],
        ...
        "plat_book": ["PLATBOOK", "PLAT_BOOK", "PB", "DEED_BK"],
        "plat_page": ["PLATPAGE", "PLAT_PAGE", "PP", "DEED_PG"],
        "just_value": ["JUSTVALUE", "JUST_VALUE", "JV", "MARKET_VAL", "JV_HMSTD"],
    ...
    # ── Build DIRECT deep-links ──────────────────────────────────────────
    # 1. OCPA Property Card – direct link to this specific parcel
    #    URL pattern: https://ocpaweb.ocpafl.org/parcelsearch/Parcel%20ID/{PARCEL_ID}
    if pid and pid != "N/A":
        ocpa_card_url = f"https://ocpaweb.ocpafl.org/parcelsearch/Parcel%20ID/{urllib.parse.quote(pid)}"
        ocpa_card_link = f'<a href="{ocpa_card_url}" target="_blank">View Property Card on OCPA →</a>'
    else:
        ocpa_card_url = "https://ocpaweb.ocpafl.org/parcelsearch"
        ocpa_card_link = f'<a href="{ocpa_card_url}" target="_blank">Search on OCPA →</a>'

    # 2. OCPA Interactive Map – direct link centered on this parcel
    #    URL pattern: https://maps.ocpafl.org/m/Home/ParcelInfo?para=&pid={PARCEL_ID}
    if pid and pid != "N/A":
        ocpa_map_url = f"https://maps.ocpafl.org/m/Home/ParcelInfo?para=&pid={urllib.parse.quote(pid)}"
        ocpa_map_link = f'<a href="{ocpa_map_url}" target="_blank">View on OCPA Map →</a>'
    else:
        ocpa_map_url = "https://maps.ocpafl.org/webmap/"
        ocpa_map_link = f'<a href="{ocpa_map_url}" target="_blank">Open OCPA Map →</a>'

    # 3. Comptroller Official Records – plat search by book/page
    #    The recorder search is at https://or.occompt.com/recorder/web/
    #    Direct search format with book/page pre-populated
    comptroller_base = "https://or.occompt.com/recorder/web/"
    if pb and pp:
        comptroller_plat_link = f'<a href="{comptroller_base}" target="_blank">Search Comptroller: PB {pb} / PG {pp} →</a>'
    else:
        comptroller_plat_link = f'<a href="{comptroller_base}" target="_blank">Search Official Records →</a>'

    # 4. Comptroller deed lookup by instrument or book/page
    if deed_inst:
    ...
    # 5. OC Tax Collector – tax bill for this parcel
    if pid and pid != "N/A":
        tax_url = f"https://www.octaxcol.com/"
        tax_link = f'<a href="{tax_url}" target="_blank">Tax Collector Lookup →</a>'
    else:
        tax_link = ""

    # Build the links block
    links_html = f"""
        <div class="field" style="margin-top:10px; padding-top:8px; border-top:1px solid #d1d9e0;">
            <span class="label" style="color:#0f4c75; font-weight:700;">🔗 Direct Links:</span>
        </div>
        <div class="field"><span class="label">Property Card:</span><span class="value">{ocpa_card_link}</span></div>
        <div class="field"><span class="label">OCPA Map:</span><span class="value">{ocpa_map_link}</span></div>
        <div class="field"><span class="label">Plat Records:</span><span class="value">{comptroller_plat_link}</span></div>
    """
...
if query_lat and query_lon:
    ...
    if parcels:
        ...
        st.markdown('<span class="status-warn">⚠ No parcel data returned – the ArcGIS service may be unavailable or coordinates are outside Orange County. Use the OCPA links below for manual lookup.</span>', unsafe_allow_html=True)
        st.markdown(f"""
        <div class="resource-card">
            <div class="res-name">Manual Parcel Search – OCPA (New Site)</div>
            <div class="res-desc">Search by address, owner name, or parcel ID directly on the current OCPA website.</div>
            <div class="res-url"><a href="https://ocpaweb.ocpafl.org/parcelsearch" target="_blank">https://ocpaweb.ocpafl.org/parcelsearch</a></div>
        </div>
        <div class="resource-card">
            <div class="res-name">OCPA Interactive GIS Map</div>
            <div class="res-desc">Click directly on a parcel to view ownership, plat book/page, and property details.</div>
            <div class="res-url"><a href="https://maps.ocpafl.org/webmap/" target="_blank">https://maps.ocpafl.org/webmap/</a></div>
        </div>
        """, unsafe_allow_html=True)
